...
**Why it matters:** The readiness probe determines when a container receives traffic. Without a Postgres readiness check, the gateway may receive webhooks before it can look up customers.
...
- Readiness probe should fail until Postgres pool is ready
...
```
- **Pros**: Won't route traffic until gateway is truly ready
...
- [ ] Readiness probe fails if Postgres is unavailable
