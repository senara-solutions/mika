...
## Problem Statement
The `/livez` endpoint checks `state.ready.load()` and returns 503 when `ready` is false. During startup (before Postgres connection, migrations, webhook registration), liveness returns 503. If the liveness probe fires before initialization completes, the orchestrator will restart the container — defeating the entire purpose of the liveness/readiness split.
...
## Findings
- **Architecture strategist**: Moderate risk — `initialDelaySeconds` might mask this, but code-level behavior is non-standard
- **Performance oracle**: Prevents unnecessary container restarts during slow migrations
...
- **Risk**: None — standard liveness pattern
...
## Resources
- Commit: 9de9ba6
- Documentation: liveness vs readiness probe semantics
