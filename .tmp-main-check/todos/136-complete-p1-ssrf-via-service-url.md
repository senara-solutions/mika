...
## Problem Statement
The Phase 3 plan stores `service_url` as freeform TEXT in the customers table and uses it directly to forward messages to containers. An attacker who gains write access to the customers table (or exploits an admin API) could set `service_url` to an internal network address (e.g., `http://169.254.169.254/` for cloud metadata, or internal services), causing the gateway to make requests to arbitrary internal endpoints.

**Why it matters:** This is a classic SSRF vulnerability. The gateway runs inside the internal cluster and can reach internal services that are not exposed externally.
...
fn container_url(customer_id: &Uuid) -> String {
    format!("http://mika-{}:8080", customer_id)
}
```
- **Pros**: Eliminates SSRF entirely, no URL stored in DB, follows deterministic service naming
