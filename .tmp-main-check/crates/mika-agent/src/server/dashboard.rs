...
/// GET /api/v1/agents/:id/budget — the LLM budget and model this agent is
/// **running under**, with the provenance of each half (mika#2457).
///
/// # What it serves, and what it refuses to do
///
/// It returns `AgentState.budget_record` verbatim — the record resolved once at
/// `init_agent`, `resolved_at` included. It does **not** re-read the disk and
/// does not recompute anything, and that refusal is the whole design rather than
/// an economy:
///
/// - Recomputing here would walk the cascade in **this** process. On a host
///   where a service variable shadows the per-agent `config.toml`, the answer
///   would name a setting that is not in force, with the authority of a
///   measurement. That is mika#2304's defect one field over (`--verbose`
///   printing the requested model while the turn ran under another) and
///   mika#2270's lesson (the server held the answer and threw it away).
/// - Refusing to re-read is also what keeps the route and the disk **two
///   distinct facts**, whose difference *is* the drift measurement mika#2457
...
/// `resolve_agent` — that would lazily construct the agent (DB, skills, task
/// engine, KG) as a side effect of a read-only observability GET. An agent this
/// process does not serve has no budget *in force* to report, and inventing one
/// would be exactly the false green U1/U2 exist to prevent. The CLI renders that
/// 404 as *"not attested"* rather than falling back to a locally computed value.
pub async fn handle_agent_budget(
    State(state): State<AppState>,
    Path(agent_id): Path<String>,
) -> impl IntoResponse {
