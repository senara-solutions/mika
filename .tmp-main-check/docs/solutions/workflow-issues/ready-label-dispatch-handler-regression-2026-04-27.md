...
- **Satisfied predicate**: `summaries.iter().any(|s| s.name == "run_claude_pilot")` — counts attempts (success or failure), not just successes. `run_claude_pilot` has known terminal failure modes (`global_dispatch_active` for concurrent webhooks, `task_not_dispatchable` for already-completed tasks, `dispatch_blocked_by` for open GitHub blockers, `dispatch_limit_exceeded` for per-turn cap). Forcing a retry on those failures would only produce misleading "never called" operator notifications. The LLM handles terminal failures via `send_message` per the prompt's Step 4. This diverges from the success-only pattern of `webhook_zero_tools` and `resume_reconcile`, but those guards don't have terminal-failure modes — `webhook_zero_tools` accepts any successful tool, `resume_reconcile` retries because reconciliation is always recoverable.
- **Correction message**: explicit instruction to call `run_gh` (issue view), `create_task`, then `run_claude_pilot` with `prompt: "<repo>#<n>"` and `task_id: <UUID>`. Mirrors the prompt's 4-step sequence so the engine and prompt agree on the contract.
