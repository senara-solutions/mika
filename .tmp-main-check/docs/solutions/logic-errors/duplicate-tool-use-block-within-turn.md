...
- **Dispatch-layer guard is the only layer that fixes execution, storage, and summary together.** Tools with side effects (`send_message` → gateway delivery, `store_fact` → memory write, `create_task` → DB row) have fired their effect before any DB constraint can trip. Guarding at the dispatch loop prevents the second execution entirely.
