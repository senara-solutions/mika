...
- **Invariant 6 — Entity round-trip with majority-vote semantics (`test_invariant_6_entity_majority_vote`):** BEFORE running migration, for 10 random `(agent_id, source_doc_path)` pairs, compute the expected post-migration entity set per agent by: (a) gathering all agents' normalized-keys for that doc; (b) computing the per-key majority-vote; (c) the agent's post-migration resolved set == majority-vote winners for that doc (union across the agent's pre-migration keys that survived the vote). Run migration. Query actual per-agent resolved entities via `kg_subject_resolutions JOIN kg_subject_entities`. **Comparison rule: normalize both sides byte-for-byte to the SQL `LOWER(TRIM(entity_key))` form.** The Rust-side expected-set computation uses exactly `s.trim().to_lowercase()` — NOT `to_ascii_lowercase()`, NOT `to_lowercase_cow()`, NOT any other half-variant. The SQL side uses `LOWER(TRIM(entity_key))`. On ASCII data (current state), these produce byte-identical output. On any future Unicode entity name, these may diverge — and if they do, the test must fail loudly at that point so a Unicode normalization contract can be chosen deliberately rather than inherited by accident. The winning row preserves its original casing in the `entity_key` column (same preservation rule the coalesce applies — only the grouping key is normalized, stored data is original-cased); the test normalizes on read. Uses `assert_eq!(sorted_normalized_keys_expected, sorted_normalized_keys_actual)` with per-agent failure messages.
