...
**Marker design + backward-compat consequence (pinned 2026-08-27, amended after review).** `FAMILY_SOUL_MARKER` does not exist on main; this ticket adds it to `crates/mika-common/src/home.rs` as `<!-- MIKA_FAMILY_SOUL_MARKER -->` and **appends** it as the last line of the `FAMILY_SOUL` constant. Three consequences to hold explicitly:

- **Tail, not head — this is load-bearing.** The plan originally said *prepend*, and the first implementation did. Review caught the defect: `build_compact_system_prompt` (`crates/mika-agent/src/prompt.rs:1289`) takes `soul_content.lines().next()` as the **entire** `## Personality` section on the MikaModel path. A leading sentinel therefore does not degrade the family persona, it *replaces* it with a platform-internal comment — on exactly the tier the marker exists to protect. Latent today (the mika#1813 carve-out notes MikaModel does not yet serve family tenants; mika#1925 tracks going live) but it fires the day it does. Pinned by `home::tests::family_soul_marker_is_not_the_persona_line`, which lives in mika-common precisely because mika-common cannot see that builder.
...
- **The marker is inert in the prompt, but not invisible.** An HTML comment carries no instruction the model would act on, and it does not trip the `family_soul_no_operator_name` invariant (`home.rs`) — it holds no operator-identity token. It *is* still prompt text, and review raised a fair doctrine question: `MIKA_FAMILY_SOUL_MARKER` tells the sealed being, quotably, that it is a "family"-tier instance of a system called MIKA, which is adjacent to what mika#1783 AC4 set out to remove. A sibling `.provisioned-tier` file would give the guard the same signal with zero prompt surface. **Deferred, not dismissed** — that is a doctrine call on #1783's intent, not an implementation detail, and it belongs with the operator. Filed as follow-up rather than folded in here.
...
- [ ] `grep -rn 'AgentTier::from_env()' crates/mika-agent/src/` returns no production hit outside the tier-resolution surface itself. **Twice corrected during implementation** — the wording is the finding, not the count. Authorized reads: `server/mod.rs` (`init_agent`, which populates the cache; and the boot-guard callsite), `server/state.rs` (the lazy-resolve guard callsite added for review finding F1), and `server/tier_guard.rs` (the message-coherence check, whose job is precisely to compare env against the passed tier). Any hit in `agent_loop/`, `teams/`, `task_engine/`, or `server/investigate.rs` is a regression — those are the per-turn consumers that must read the cache. See `todos/1962-injection-verification.md` § I3.
...
## Review findings and resolutions (2026-08-27)

`/ce:review` on the implementation commit raised six findings; all six were verified against the code before acting. Five were fixed in this ticket, one deferred.

| # | Severity | Finding | Resolution |
|---|---|---|---|
| F1 | HIGH | `AppState::resolve_agent`'s lazy-construction path (mika#1399) calls `init_agent` at *request* time and re-reads the tier from the server's env, never consulting the guard. Re-opens the drift class for any agent that appears after boot. | **Fixed.** New `tier_guard::check_agent_tier_consistency` (per-agent form) called in the slow path before construction; on mismatch the agent is declined with an `error!` rather than served. Declining ≠ aborting: one drifted agent must not stop the healthy ones. |
| F2 | MED-HIGH | The prepended marker becomes the whole `## Personality` section on the compact prompt path, replacing the family persona with a platform-internal comment. | **Fixed.** Marker moved to the tail of `FAMILY_SOUL`; invariant pinned by `family_soul_marker_is_not_the_persona_line`. Doctrine sub-point (prompt surface at all) deferred — see § Marker design. |
| F3 | MEDIUM | The guard enumerated via `agent::list_agents` (requires `config.toml`), but `resolve_agent` serves anything with `identity.toml` and per-agent `config.toml` is `.required(false)`. An agent with identity but no config was servable and invisible to the guard. | **Fixed.** `servable_agent_names` takes the union of both predicates. |
| F4 | MEDIUM | Malformed-identity fail-fast was mis-scoped: an unrelated typo in an *operator* agent's `identity.toml` downed the whole process, a failure mode that did not previously exist. | **Fixed.** Hard failure narrowed to files that still carry family signal (raw-text scan for the `FAMILY_AGENT_SKILL_ALLOWLIST` names). No signal → `Ok(false)` + `warn!`. Keeps the security property, drops the availability cost. |
| F5 | LOW-MED | The guard's doc claimed purity over (disk, tier) while the error message read the live env, so it could render `MIKA_AGENT_TIER=family … resolves to Default tier` — self-contradictory, and it points the operator at an already-correct variable. | **Fixed.** The displayed value is derived from the passed `tier`; two tests cover the agreeing and disagreeing cases. |
| F6 | LOW | mika-cli reads `from_env()` at three sites rather than one. Not a bug (a process environ is stable) but two disciplines coexist in one file. | **Deferred.** Not a defect, and it is mika-cli hygiene rather than this ticket's surface. A `OnceLock`-backed accessor would remove the class; filed as follow-up per `feedback_implementation_scope_bundling`. |

Findings the review checked and cleared: the `teams/engine.rs` pre-spawn `let tier = self.tier;` hoist (correct — `AgentTier` is `Copy`, and it sits with the other pre-spawn locals); `server/investigate.rs` borrowing the default agent's tier (sound, and unconditionally so once F1 is fixed); argument-order safety across the threading sites (a positional slip would not compile — `AgentTier` is type-distinct from every neighbouring param); and `provision_well_known_agents` under a family env (well-known agents overwrite both identity and soul, so they never retain family templates).
