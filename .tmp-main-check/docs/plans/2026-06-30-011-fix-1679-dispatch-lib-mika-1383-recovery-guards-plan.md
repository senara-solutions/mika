...
The rev-1 plan added a `wip()` marker commit (its "Edit 3") specifically to make Guard 2 fire. **RATIFIED (mika-arch second-pass, session `2d397bee`, Option (a)):** relocate that single defense-in-depth commit into **Path B's `commit-pushed-no-pr` branch** — an empty `git commit --allow-empty -m "wip(mika#1383): auto-PR-create rescue for ${REPO}#${ISSUE_NUM}…"` + `git push` immediately before Path B's `gh pr create` (2519), guarded to the `commit-pushed-no-pr` class **only** (the `dirty-worktree` class is already `wip()`-prefixed and must not get a second empty commit). This restores all three guard layers on the single-source-of-truth path, matching rev-1's defense-in-depth intent without re-introducing duplication. Architect rationale: contract churned 4× this week → safety-critical operator-review path warrants defense in depth; empty-commit cost negligible; three consistent guards across all rescue paths simplifies the mental model.
...
- **AC6 (Guard 2 — Edit 2 RATIFIED, Option (a))** — Path B's `commit-pushed-no-pr` branch adds an empty `wip(mika#1383): …` marker commit (+ push) before `gh pr create`, guarded to that class only. After a `commit-pushed-no-pr` rescue, `gh pr view <N> --json commits --jq '.commits[-1].messageHeadline'` returns a string starting with `wip(mika#1383):`, so Guard 2's `isDraft AND ^wip\(` conjunction fires. The `dirty-worktree` class is NOT given a second empty commit (it is already `wip()`-prefixed). Verified by reading the PR diff (the marker-commit step is inside the `commit-pushed-no-pr` guard) + test/trace evidence.
