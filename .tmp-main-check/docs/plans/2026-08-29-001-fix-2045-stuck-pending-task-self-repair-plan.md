...
- R3b. Le wrapper de remplacement porte la classe de dispatch de sa parente.
...
- R6. Une tâche orpheline est d'abord ré-armée. Elle n'est expirée que si la réparation est définitivement impossible : budget épuisé, ou dispatch non reconstructible.
- R6b. Un refus de réparation pour une condition passagère — file différée pleine, erreur de base — ne fait pas expirer la tâche. Elle est laissée intacte, sans consommer de budget, et retentée au passage suivant.
...
- KTD6. **Le refus de réparation est typé, pas booléen.** `rearm_deferred_callback` renvoie `RearmOutcome::{Rearmed, NotNow, Unrepairable}`. Un booléen forcerait le ramasseur à lire « la file différée est pleine » comme « cette tâche est irréparable », et à détruire pour une condition passagère une tâche qui avait encore du budget. Seul `Unrepairable` autorise l'expiration. Gouverne R6, R6b.
- KTD7. **La réparation entre dans la classe de la parente, jamais dans une classe supposée.** Une issue non groomée appartient à la classe `groom` ; la ré-armer en `implement` mettrait en file un `dev-pilot` pour un travail qui a encore besoin de `dev-groom`, et occuperait le mauvais créneau pour le faire. `OrphanedPendingTask` porte donc `dispatch_class`. Gouverne R3b.
...
- **Test Scenarios** (`crates/mika-agent/tests/eval/test_deferred_wrapper_rearm.rs`, nouveau) :
  ...
  - Même cas avec `stuck_rearm_count` déjà au budget → `Unrepairable`, aucun wrapper créé.
  ...
  - Ré-armement au plafond `MAX_PENDING_DEFERRED_CALLBACKS` → `NotNow`, sans insérer et sans consommer de budget.
...
- **Test Scenarios** (`crates/mika-agent/src/task_engine/engine.rs`, module de tests, à côté des tests `promote_pending_deferred_if_idle` existants) :
  ...
  - Parente orpheline dont la réparation est refusée pour plafond de file → **laissée intacte**, budget non consommé, retentée au passage suivant.
  - Parente orpheline de classe `groom` → le wrapper de remplacement porte `dispatch_class = groom` et rejoue `dev-groom`.
...
- [ ] Un refus de réparation pour file pleine laisse la tâche intacte, sans consommer de budget.
- [ ] Le wrapper de remplacement porte la classe de dispatch de sa parente.
