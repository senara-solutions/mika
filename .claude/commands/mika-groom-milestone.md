...
9. For each open sub-issue (in ascending issue-number order):
   ...
   **3b.** Capture the per-sub-issue outcome:
   ...
   - `plan_path`: the committed plan file path (e.g., `docs/plans/<YYYY-MM-DD>-<NNN>-<type>-<slug>-plan.md`)
...
13. Parse the response. Look for:
    ...
    The `Disposition: <KEYWORD>` MUST be the literal final line of the response per literal-final-line discipline (`mika/docs/solutions/best-practices/mika-arch-first-dogfood-2026-04-25.md`).
...
14. This phase does NOT route to mika-arch. Per the recursive self-review carve-out (`mika/docs/solutions/best-practices/recursive-self-review-carve-out-2026-04-26.md`, now codified in `mika/docs/architecture/review-guide.md` section 7), when a milestone touches mika-arch's own operational surface or when the reviewing agent is structurally vested in the outcome, the second-pass routes to an external reviewer.
...
16. Emit a clear pause-and-ask to the operator:
    ...
    (mika/docs/architecture/review-guide.md section 7), the second-pass for milestone grooming
...
18. Draft the sequencing record using the template at `mika/docs/plans/templates/milestone-sequencing-record-template.md`. Populate all sections from Phase 4 output:
...
23. Post a comment on the milestone's tracking issue (or first sub-issue) summarizing:
    ...
    To proceed: add the `ready` label on each sub-issue in order per sequencing
    record (canonical positive-consent per mika#841). Operator-only step.
    ```
...
- **Recursive self-review carve-out.** Second-pass routes to an external reviewer when the milestone touches the architect's own operational surface (per `mika/docs/architecture/review-guide.md` section 7).
...
- `mika/docs/plans/templates/milestone-sequencing-record-template.md` — sequencing record schema used in Phase 6.
- `mika/docs/architecture/review-guide.md` section 7 — self-review boundary (codified carve-out rule for when second-pass routes external).
- `mika/docs/solutions/best-practices/recursive-self-review-carve-out-2026-04-26.md` — historical evidence and original carve-out reasoning.
