...
## Problem Statement
The single `/health` endpoint runs a Postgres `SELECT 1` on every probe (routes.rs:346-356). Under load, health probes compete for the 10-connection pool. A false 503 from pool exhaustion triggers K8s pod restarts, amplifying the problem.
...
```rust
// /livez: process alive, no DB (for K8s livenessProbe)
...
// /readyz: DB check (for K8s readinessProbe)
...
```
- Effort: Small (15 min)
- Risk: Low — K8s manifests must be updated to use new endpoints
