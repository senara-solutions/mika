...
Bundled skill handler scripts are set to `0o755` at `bundled_skills.rs:225`. On multi-user systems, any user can read and execute these scripts. While they contain no secrets and containers are single-user, `0o700` would be consistent with the `bootstrap()` function which sets directories to `0o700` and files to `0o600`.
