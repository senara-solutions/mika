...
- **Observer fan-out (mika#1403):** `observe_event()` in `github.rs` returns a `Vec<ObserverNotification>` of observer agents (v1 hardcodes `mika-prime` as the sole observer) that should receive a **filtered, best-effort, `internal=1`** copy of bearing-moving events — supplementary to, never replacing, the primary `route_event()` route. The no-noise filter (Delta 2) emits only for: `pull_request.closed(merged=true)` → `backlog_changed`, `issues.closed` → `backlog_changed` (**observer-only** — `route_event` drops it to None; the fan-out is placed BEFORE the not-routable early-return so it is still captured, AC8), `check_suite.completed(success)` → `dispatch_gate`, `check_suite.completed(failure|timed_out)` → `loop_health`, `pull_request.opened` → `dispatch_gate`. All routing/label/comment/push/review churn returns empty. Observer copies target the fixed session `OBSERVER_SESSION_ID` (`00000000-0000-0000-0000-000000000000`, mika#1401 single-session) and carry `session_id` + `internal:true` in the `/message` payload; the agent handler stores them as `internal=1` and skips the structural verdict/CI/ready-label handlers (observer turns are plain LLM turns). Delivery is intentionally lighter than the primary path (`deliver_observer_event`): single resolve + POST, shed on semaphore pressure, WARN-and-drop on failure, **never a DLQ entry** (AC4) — a lost observer copy is re-driven by the next relevant event. Structured log event `observer_event_dispatched` (fields: `event_type`, `action`, `event_class`, `observer_agent`, `delivery_id`) instruments the fan-out for post-ship filter tuning (AC10). Observers share the primary `webhook_semaphore` and are shed first, so fan-out never delays the primary dispatch or the 200 to GitHub (AC3). **mika-prime setup (operator-side, AC7):** disable its heartbeat (`[heartbeat] enabled = false` in `~/.mika/agents/mika-prime/identity.toml`) — observer events replace the clock cadence; this is a runtime config step, not shipped in the repo.
