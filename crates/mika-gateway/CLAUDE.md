...
- `pull_request.opened/synchronize/review_requested/ready_for_review` -> mika-qa (mika#1822: `ready_for_review` fires when a draft PR is un-drafted; it is primary-only — no `secondary_targets` fan-out — and is *not* subject to the `review_requested` reviewer filter below since a draft→ready toggle carries no `requested_reviewer`)
