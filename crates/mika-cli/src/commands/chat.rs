...
async fn spawn_agent_worker(
    ...
    // mika#2290 — same boundary, same reason: the CLI process start is where
    // the posed hosting fact is read, once.
    let cli_deployment = mika_common::home::Deployment::from_env();
