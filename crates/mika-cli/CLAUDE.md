...
- `mika agents budget [--agent <name>] [--format text|json|yaml]` — Show the LLM budget and model an agent is **running under**, with the provenance of each half (mika#2457). Reads `GET /api/v1/agents/{id}/budget` on mika-spirit, which serves the record frozen at `init_agent`; the first line carries `resolved_at`, so the output says *when* it was true rather than only what it is.
  - **Nothing is resolved locally, and that is the whole point.** `BudgetProvenance::resolve` reads the `process_env` of the process that calls it, so a subcommand computing its own answer would print `agent_config` at 240 while mika-spirit runs at 300 under a service variable — a field asserting, with authority, a setting that is not in force. That is mika#2304's defect one field over. **Spirit attests, the CLI renders; never the reverse.** Held structurally by `mika2457_the_cli_resolves_no_budget_locally`, a source scan whose allowlist is shipped **empty**: when it fires the second reader is removed, not listed, because an allowlist that stops being empty here *is* that false green reintroduced.
  - **Server unreachable or 404 prints *"not attested"* and no values.** That population is ambiguous — a daemon that is down, a binary predating the fix, an agent this server does not serve — and the ambiguity is preferable to a wrong number; mika#2304 had to name the same population for the same reason. In JSON it is `{"attested": false}` with no budget fields, never a fabricated record. **The cause is still named**, because "no values" does not license a false explanation: `reason` is `unreachable` / `unauthorized` (401/403 — this shell's `MIKA_INTERNAL_TOKEN` is not the server's) / `not_found` (404, the one genuinely ambiguous case) / `http_error` / `malformed_response` (a 2xx the CLI's mirror cannot read — version skew), plus `status` when the server answered; the text render says the same in one line.
  - Reads three worlds apart, which is what the mika#2457 probe exists for: `http_source = agent_config` means the setting is in force and a cut comes from elsewhere; `process_env` means a fleet-wide variable shadows the per-agent `config.toml`, and the remedy is removing it from the service environment; `default` means the file was never read or never carried the key, and the remedy is a provisioning gesture. An unreadable `llm_provider` renders as `model (non résolu — provider illisible : "<brut>")` rather than an empty model — mika#2328's `unknown_provider`, pre-existing and only carried through here.
  - **The `code` line (mika#2473) names the provider when it moved, and the key to edit.** `ModelDriftCheck::compare` returns `Drift` as soon as the **provider** differs, model string identical included, so a provider-only drift used to print « DÉRIVE : le runtime sert <la même chaîne> » — a contradiction with no visible cause. That state is reachable, not theoretical: mika-qa on this workstation is in provider drift (`zai` → `openrouter`) at the same `glm-5.2`. The line now adds `, fournisseur <déclaré> → <en vigueur>` **only when the two differ** (printing two identical values would read as a second change), and `, clé: <model_config_key>` — the key an operator would actually edit, which reached neither the text render nor the JSON/YAML mirror before. The key clause is **suppressed on an empty key**, which is exactly the `unknown_provider` shape: there is no model key to name there, and naming a plausible one would be a false provenance. « en phase » still appears on the `in_sync` arm and nowhere else — the negative control that keeps the four drift states distinguishable rather than merely present.
