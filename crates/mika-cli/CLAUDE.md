...
- **Verbose-gated** (require `--verbose`): `session_id` (String), `model` (String, `provider/model` format), `agent_id` (String), `latency_ms` (u64, wall-clock ms for the agent loop), `tokens` (object: `{input, output, cache_read, cache_write}` — all `Option<u64>`, mapped from `LlmUsage`; absent when the agent loop returns no usage).
