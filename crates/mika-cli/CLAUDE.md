...
- `mika tasks list` — List active tasks (pending, in_progress, recurring_active). Bare `mika tasks` is an alias. Supports `--format text|json|yaml`.
- `mika tasks get <id>` — Show full details for a single task. Supports `--format text|json|yaml`.
