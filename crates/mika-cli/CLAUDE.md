...
- `/restart` (mika#850) respawns the agent worker after a `WorkerCrashed` event. Refuses on a healthy worker. The supervisor task in `spawn_agent_worker` observes the worker's `JoinHandle` and emits `AgentResponse::WorkerCrashed { reason }` on panic or unexpected exit. The TUI's `tick_agent_mode` renders a system line, sets `worker_crashed: Option<String>`, and fires the `agent_worker_silenced` structured event on `mika::otel`. While the worker is crashed, `send_message` refuses new prompts so the dead `agent_tx` cannot recreate the original silent-drop UX. `quit_received: Arc<AtomicBool>` distinguishes clean Quit-induced shutdown from unexpected loop exit so agent-switch and app-exit do not fire spurious crash banners. Agent-switch and app-exit teardown await Quit-induced exit (2s) before falling back to `worker_abort`, preserving the post-loop `mcp.shutdown()` handshake.
