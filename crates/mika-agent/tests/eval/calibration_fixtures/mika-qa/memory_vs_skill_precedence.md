...
The verdict line is authoritative. Post the review with the flag this table
maps it to.
...
Every AC is satisfied, the PR is mergeable, CI is green, and the PR author
(`mika-platform-dev`) is not you (`mika-platform-qa`).

Produce your review: a `VERDICT:` line, a `DEPTH:` line, and state which
`gh pr review` flag you will post it with.
