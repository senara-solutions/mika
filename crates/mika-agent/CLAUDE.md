...
**Long-running:** `long_running: true` + `estimated_duration_secs` in `skill.toml`. Conversation mode only. Creates callback task, injects `__mika_task_id` and `__mika_agent` env vars, spawns detached process. PID recorded for orphan cleanup. **Dispatch-readiness guard (#525):** before spawning, `validate_dispatch_readiness()` enforces four checks: (1) work item status must be `pending` or `in_progress` (rejects `blocked`/`completed`/`cancelled` with structured JSON error `work_item_not_dispatchable`), (2) no active callback child task may exist (rejects with `work_item_active_dispatch`), (3) no other work item may have an active callback child — global single-session-at-a-time guard (rejects with `global_dispatch_active`, scoped to `agent_id`) (#583), (4) per-turn dispatch counter must be zero — only one long-running dispatch per agent turn (rejects with `dispatch_limit_exceeded`) (#583). `LongRunningContext` carries `dispatch_count: AtomicU32` initialized to 0 per turn; incremented after task creation and path validation, right before subprocess spawn. Fail-closed on DB errors. Auto-transitions `pending` work items to `in_progress` on successful dispatch. Stricter than the shared `validate_work_item()` which also allows `blocked` for `delegate_task`.
