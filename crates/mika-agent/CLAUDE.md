...
- **Threshold:** `Settings::effective_pilot_cost_alert_usd()` → `pilot_cost_alert_usd` / `MIKA_PILOT_COST_ALERT_USD`, default `DEFAULT_PILOT_COST_ALERT_USD` = 40. Prefixed, unlike its `PILOT_MAX_TURNS` sibling, because this one is read by **mika-spirit**, where `scrub_mika_env_vars` does not run.
