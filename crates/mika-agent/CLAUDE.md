...
3b. **PR review early-accept (#695):** `has_successful_pr_review()` checks if `all_tool_summaries` contains a successful `run_gh` call with `"pr"` and `"review"` in the input. When true, guards #4–#7 are skipped — the qa-review workflow's primary action completed and forced continuation would risk duplicate submissions. Defense-in-depth: `run_gh` also tracks `ToolContext.pr_review_posted` (AtomicBool per turn) and rejects duplicate `pr review` calls with a structured `duplicate_pr_review` error.
...
**Current: v26.** Tables: sessions, messages (with `internal` flag for agent-to-agent visibility), team_workspace, audit_events, skill_overrides (with `enabled` column for DB-backed disable state), tasks (with manual/callback/a2a trigger types and a `type` column distinguishing `issue`/`milestone`/`project`), a2a_task_map, a2a_artifacts, a2a_push_notification_configs, llm_calls, tool_calls, team_runs. Knowledge Graph tables: `kg_entities` (global typed nodes with CHECK-enforced `entity_key = type:name`, entity types include `person`, `org`, `project`, `place`, `concept`, `event`, `skill`, `tool`, `agent`, `problem_type`; `properties_json TEXT` for structured metadata), `kg_relationships` (directed edges with FK cascade), `kg_chunks` (per-agent text chunks with dedup key), `kg_subject_entities` (confidence-scored mentions), `kg_subject_resolutions` (subject→domain mapping), `kg_subject_relationships` (fact triples), `kg_chunk_subjects` and `kg_chunk_subject_relationships` (provenance junction tables), `kg_extractions` (extraction tracking), `kg_resolutions_log` (resolution audit trail). `unified_timeline` VIEW for cross-subsystem queries. Session-based message storage with FK. System sessions (`system-{agent_id}`) for compaction.
...
- v24->v25: Knowledge Graph schema foundation (#722). 10 new tables across three layers: **Domain** (global `kg_entities` with `entity_key = entity_type || ':' || name` CHECK, `kg_relationships` with FK cascade), **Lexical** (per-agent `kg_chunks` with `(agent_id, source_doc_hash, chunk_index)` dedup, designed to compose with `search_content` via `source_type='kg_chunk'`), **Subject** (per-agent `kg_subject_entities` with confidence bounds, `kg_subject_resolutions` with resolution type CHECK, `kg_subject_relationships` fact triples), **Provenance** (`kg_chunk_subjects`, `kg_chunk_subject_relationships` junction tables), **Tracking** (`kg_extractions` with status CHECK, `kg_resolutions_log` with outcome CHECK). Constants and helpers in `crate::kg_schema`. See `docs/architecture/kg-id-convention.md`.
- v25->v26: Domain graph entity types and `properties_json` (#687). Rebuilds `kg_entities` table to widen entity_type CHECK constraint (adds `skill`, `tool`, `agent`, `problem_type`) and adds `properties_json TEXT` column. Table rebuild via rename→create→copy→drop preserves rowids for FK integrity. New constants in `kg_schema.rs`: `ENTITY_TYPE_SKILL`, `ENTITY_TYPE_TOOL`, `ENTITY_TYPE_AGENT`, `ENTITY_TYPE_PROBLEM_TYPE`, `KG_DOMAIN_ENTITY_TYPES`, `REL_DEPENDS_ON`, `REL_PROVIDES`, `KG_DOMAIN_RELATIONSHIP_TYPES`. New `crate::kg` module with `DomainGraphBuilder` — deterministic startup-time population of domain entities and `DEPENDS_ON`/`PROVIDES` edges from `SkillRegistry`, `ToolRegistry`, `McpManager`, and agent configs.
