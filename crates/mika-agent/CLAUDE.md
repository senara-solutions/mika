...
Ten sequential post-conditions on assistant text responses, plus one early-accept:
...
4b. **Milestone-close-claim guard (#797):** `detect_milestone_close_claim_without_patch()` detects when assistant text claims a GitHub milestone is closed (regex `\bmilestone\b.{0,80}\b(closed|close)\b`) without a satisfying `run_gh` call in the turn. A qualifying call has `input_summary` containing `"api"`, `"PATCH"`, `state=closed`, and a milestones API path. Single retry tracked via standalone `milestone_close_claim_retry_done` flag. Composes with #4: when both regexes match (e.g., "completed and closed"), #4 fires first; #4b fires on a subsequent EndTurn after #4 is satisfied. After the retry, second-violation in the same `run_loop` emits a `warn!` and accepts the EndTurn (single-retry budget exhausted).
