...
6. **Webhook zero-tools guard (#696):** If the user message starts with `[GitHub]` (a webhook event) and zero successful tool calls were made in the turn, the response is rejected once. Enforces that webhook events require action — text-only responses to webhooks are fabrications. Uses `all_tool_summaries.iter().any(|s| s.success)` to distinguish "tried but failed" from "never tried."
