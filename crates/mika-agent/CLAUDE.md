...
Six sequential post-conditions on assistant text responses:
...
**Transient always_on override (#670):** `SkillRegistry::apply_transient_always_on(skill_names)` sets `always_on = true` on named skills for a single CLI invocation. Called after `apply_overrides()` so it stacks on top of DB state. Returns `TransientOverrideResult` with separate `disabled` and `not_found` lists for accurate caller warnings. Cannot resurrect disabled (evicted) or skipped skills. Used by `mika ask --skill-always-on <name>` (repeatable). Not persisted.
