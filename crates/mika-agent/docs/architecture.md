...
| `llm_calls` | LLM API call tracking (provider, model, tokens, latency, stop_reason, status, step, `prompt_variant` JSON — resolved skill variant descriptors). 30-day retention. |
| `tool_calls` | Tool execution tracking (full input/output at 50KB cap, tool source, skill name, timing). 30-day retention. |
| `unified_timeline` | VIEW — UNION ALL across messages, audit_events, tasks, team_workspace, llm_calls, tool_calls for cross-subsystem correlation by trace_id (task leg uses `COALESCE(execution_trace_id, created_trace_id)`) |
