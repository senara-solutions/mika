...
| `kg_docs_roots` | `Option<Vec<PathBuf>>` | None | `MIKA_KG_DOCS_ROOTS` | Colon-separated list of absolute docs-root paths for multi-corpus agents (#798). Per-agent `[kg].docs_roots` in identity.toml takes precedence. Linux/macOS only. |
...
| `spirit_log_file` | `Option<PathBuf>` | None | `MIKA_SPIRIT_LOG_FILE` | File path for mika-spirit log output. **When set in `json` format, logs go to the file *only* — the JSON stdout layer is not installed (mika#2195).** A launcher that redirects stdout into this same file (OpenRC `supervise-daemon --stdout /var/log/mika/server.log`) would otherwise write every event twice, and every consumer that counts events over-counts ×2. In `pretty` format the console layer is kept (dev convenience; no launcher redirection in play). |
...
| `MIKA_KG_DOCS_ROOTS` | No | Colon-separated list of docs-root paths for multi-corpus agents (#798; per-agent `[kg].docs_roots` takes precedence; Linux/macOS only) |
