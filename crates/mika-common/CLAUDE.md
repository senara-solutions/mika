...
Multi-provider via `LlmProvider` trait — 11 supported providers, each with its own `model`, `api_key`, and `base_url` fields in config. Active provider selected by `llm_provider` config key. Providers: Anthropic (default, Claude Sonnet 4.6), OpenAI, OpenRouter, Groq, Ollama, Mistral, Google (Gemini), DeepSeek, MiniMax, Kimi, Qwen. Anthropic uses its native API client. Ollama uses a native provider (`llm/ollama.rs`) that targets `/api/chat` — no `/v1` suffix required in the base URL, supports tool calling natively (synthetic IDs since Ollama omits them), no vision support (deferred). All other non-Anthropic providers use the `OpenAiCompatibleProvider` adapter.
