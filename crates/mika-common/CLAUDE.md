...
Multi-provider via `LlmProvider` trait — 13 supported providers, each with its own `model`, `api_key`, and `base_url` fields in config. Active provider selected by `llm_provider` config key. Providers: Anthropic (default, Claude Sonnet 4.6), OpenAI, OpenRouter, Groq, Ollama, Mistral, Google (Gemini), DeepSeek, MiniMax, Kimi, Qwen, Z.AI (GLM models, OpenAI-compatible), MikaModel. Anthropic uses its native API client. Ollama and MikaModel both use the native Ollama provider (`llm/ollama.rs`) that targets `/api/chat` — no `/v1` suffix required in the base URL, supports tool calling natively (synthetic IDs since Ollama omits them), no vision support (deferred). MikaModel is a separate `ProviderKind` so the operator can configure the internal endpoint (`mikamodel_*` keys, defaults to `http://localhost:11434` + model `"mika"`) independently from a general-purpose Ollama (`ollama_*` keys); the same transport, different config namespace. All other non-Anthropic providers use the `OpenAiCompatibleProvider` adapter.
