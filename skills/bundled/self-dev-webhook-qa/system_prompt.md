...
3. **Act on the verdict:**
   ...
   **pass** — Merge immediately, then correlate:
   ...
   > **NEGATIVE-TEST consistency check (mika#2264) — read the review body you already have; no tool call, no narration.** `qa-review` Step 2.5.4b requires every verdict body to carry exactly one `NEGATIVE-TEST:` line. Before merging, scan the review body in this message for it:
   >
   > - `NEGATIVE-TEST: satisfied|mechanical|n/a` → consistent with `pass`. Merge as described below.
   > - `NEGATIVE-TEST: missing` → **contradiction.** `missing` is only consistent with `block[ac]`; a `pass` alongside it means the reviewer named an unsatisfied gating AC and approved anyway. Do NOT call `pr_merge_with_gate`. Correlate to task (Step 4), keep it `in_progress`, and notify Vincent: "{repo}#{number}: mika-qa emitted `pass` with `NEGATIVE-TEST: missing` — contradictory verdict, merge withheld. {PR URL}". End the turn.
   > - **Line absent entirely** → the reviewer did not run 2.5.4b, so its result is unknown, not satisfied. Do NOT call `pr_merge_with_gate`. Correlate to task (Step 4), keep it `in_progress`, and notify Vincent: "{repo}#{number}: `pass` verdict carries no `NEGATIVE-TEST:` line — 2.5.4b not evidenced, merge withheld. {PR URL}". End the turn.
   >
   > This is the downstream half of a two-layer gate: the pilot owes the negative test (`self-dev` Rule 13), the reviewer owes the refusal (`qa-review` 2.5.4b), and this check catches a reviewer that skipped its own rule. The Evidence → Action discipline is unchanged — the evidence is in this message, and withholding a merge IS the action. Narrating without acting remains a workflow failure.
