...
Use `run_claude_pilot` with `skill: "dev-pilot"` to dispatch a headless Claude Code implementation session via the claude-pilot CLI.

### When to use
- User wants to run Claude Code on a project (implement a feature, fix a bug, refactor, etc.)

### How it works
1. Call `run_claude_pilot` with `skill: "dev-pilot"` and a `prompt` in `repo#number` format (e.g., `mika-skills#8`)
...
### Important
- **Always pass `skill: "dev-pilot"`** — the tool requires an explicit skill argument
