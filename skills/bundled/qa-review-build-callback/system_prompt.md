...
> **Post-callback discipline (mika#991, enforcement mika#2355):** After processing the build callback, act on the result immediately — do NOT narrate state and ask for confirmation. Either continue to AC execution and verdict posting, or emit `block[pipeline]`/`block[ac]` as appropriate. What the engine enforces on this turn, precisely: an EndTurn with no successful `run_gh` call carrying `pr review` in this turn's tool history is rejected **once** and re-prompted with the `qa_build_callback_verdict` correction (naming `run_gh pr review` and the `VERDICT:` line). It does **not** require `update_task_status` or `send_message` on this turn — that is the contract of a pilot-dispatch callback, not of a build callback — and a `send_message` saying "Build succeeded" satisfies nothing here.
...
1. **Mandatory plan re-read (Step 2.5.1 + 2.5.2 + 2.5.3, ALWAYS).** Re-derive the worktree path from the prior turn's `qa_pr_view` output (still in conversation context as a tool result, even under partial compaction) — same formula as Step 3e.2: `sanitized_branch = headRefName with "/" → "-"`; `worktree = ~/workspace/mika-platform/.claude/worktrees/${sanitized_branch}/mika/` — written out in full, because no variable is expanded on a tool argument (`run_shell` scrubs every `MIKA_*`, and the `build_mika` handler's environment never carries one). Then re-read the plan and re-extract ACs unconditionally:
   ```
