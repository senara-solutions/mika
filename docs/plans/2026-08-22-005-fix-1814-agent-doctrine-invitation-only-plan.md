...
- [ ] **AC11.** (added first-pass architect ITERATE F1) The canonical bearing memory `project_mika_invitation_only_no_public_launch.md` MUST be operator-authored (Vincent) before PR merge. The prompt template references the memory by name — the file's existence is a merge-gate precondition, not a code-PR deliverable. If the memory is still missing at PR-merge time, mika-qa blocks with an operator-review label. This preserves the operator-authored institutional-memory lifecycle (review-guide.md § Orthogonality) and avoids drift between code enforcement and memory source-of-truth.
...
3. qa-review's PR gate MUST verify the memory file exists on the reviewer's filesystem (`test -f`) before granting `VERDICT: pass`. Rationale: the doctrine cannot land in production without its bearing memory anchor.
