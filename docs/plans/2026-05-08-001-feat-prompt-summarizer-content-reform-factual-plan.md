...
**Per architect second-pass NF4:** the omission of a live-summarizer-eval test in this PR is **intentional**. AC#2 asserts the prompt content (the producer side); the integration scenario asserts the consumer-side response shape on a fixture-supplied summary. The third loop — does the actual LLM running through `summarize_messages()` produce Fact/Decision/Outcome shape on real content? — is deferred to deploy smoke per the runbook §3 (engine-semantic class probe) and to weekly calibration runs (`MIKA_EVAL_REAL_PROVIDERS`). Including a real-LLM unit test would gate CI on summarizer output shape across multiple providers, multiplying CI cost and surface-area for flakes; deferring to deploy smoke matches the existing pattern for prompt-content changes.
...
- **R5 (low) — `Open:` prefix truncation-priority risk (per architect second-pass NF5):** the prompt does not instruct the summarizer to place `Open:` bullets before `Fact:` bullets. Under Axis 3 silent-mode truncation, `Open:` items near the end of the summary may be lost if the budget cuts before them. If session-continuity regressions are observed post-deploy (an agent loses track of pending questions across compaction), the fix is a prompt-level ordering instruction (e.g., "List `Open:` bullets first") — not a code change. Out of scope for this PR; revisit on observed signal.
