...
2. **Specified location has no delegation results.** mika#1652's plan placed the check "immediately after `parse_task_assignments()` returns, before spawning the join_set" — but `parse_task_assignments()` (engine.rs:1573) produces task *assignments*, not delegation *results*. Delegation outcomes only exist after the join_set collection loop (engine.rs:1242-1281) — and at that layer, each result is stringified into `TaskStatus::Failed(String)` (line 1252), so no structured error survives.

3. **The founding-incident shape never produces `TaskStatus::Failed`.** `a2a_call` 503s return `ToolOutput::error(...)` — a *tool result inside the agent loop* (tools/a2a_call.rs:131-138). The agent's `run_loop` returns `Ok(response)`, so the delegation completes as `TaskStatus::Completed`, not `Failed`. An "all-delegations-failed" check at the join_set boundary would NEVER fire on the motivating case (Litha team `d68fdcaf-…`).
...
Architect-confirmed: semantic clarity worth the enum extension cost. Mark with `#[serde(rename = "failed_transport")]` for DB string-form compatibility. Reusing `RunStatus::Failed` would blur the distinction between "all members transport-error (retryable in principle)" and "mixed/business-logic failure" — dashboards and metrics benefit from the distinction.

## Fix shape (skeleton — architect fills in via D1/D2/D3)

```
After join_set collection loop completes:
  ...
  3. Else: proceed to review phase (current behavior).
```
...
4. **Wire short-circuit at engine.rs:~1281** per D3.
...
- `crates/mika-agent/src/teams/engine.rs:1242-1290` — short-circuit insertion point + new detection logic
- `crates/mika-agent/src/teams/engine.rs:1573+` — `parse_task_assignments` (referenced for context, NOT edited)
- `crates/mika-agent/src/teams/types.rs` (or wherever `RunStatus` lives) — new terminal state variant (architect-bearing)
...
- `crates/mika-agent/src/teams/engine.rs:1240-1290` — result collection loop (short-circuit target)
- `crates/mika-agent/src/teams/engine.rs:1573` — parse_task_assignments (mika#1652's incorrect target)
- `crates/mika-agent/src/tools/a2a_call.rs:131-138` — where 503 becomes `ToolOutput::error` (the signal source)
