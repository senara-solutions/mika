...
- R2. Le message d'échec de convergence porte cette raison, et n'affirme la présence d'un plan sur la branche que lorsqu'un appel à `_committed_plan_on_branch` l'a confirmée.
...
- R5. Une session du pilote principal — celui lancé par `_run_claude_pilot` — dont `status` vaut `terminated` est classée comme échec de session **avant** que `_post_flight_recovery` ne s'exécute ; le message nomme le garde-fou déclencheur et le nombre de tours atteints.
- R6. Sur session terminée, `_iterate_groom_loop` n'est jamais invoqué. Le push est sauté **uniquement lorsque la branche ne porte aucun commit en avance sur son point de départ** — une session tuée après avoir commité conserve son travail.
- R7. Le callback d'une session terminée porte une seule ligne `Outcome:`, `PIPELINE_INCOMPLETE` avec la cause de terminaison, et aucun diagnostic de contenu.
...
- KTD2. **Le garde de session terminée vit à l'intérieur de `_run_claude_pilot`, entre l'extraction de `STATUS` (`:1236-1249`) et l'appel `_post_flight_recovery` (`:1280`).** C'est le seul point qui satisfait R7 : `_post_flight_recovery` est imbriqué dans `_run_claude_pilot`, donc il produit ses diagnostics de contenu **avant** tout retour dans `dispatch_claude_pilot`. Un garde placé après l'appel arriverait trop tard et ne pourrait que préfixer un texte déjà faux. Gouverne R5, R6, R7.
...
- [ ] La présence d'un plan sur la branche n'est affirmée qu'après un appel réussi à `_committed_plan_on_branch`.
...
- [ ] Une session `STATUS=terminated` est classée avant l'exécution de `_post_flight_recovery`, et ne déclenche pas `_iterate_groom_loop`.
- [ ] Le callback d'une session terminée nomme le garde-fou et le nombre de tours atteints, et porte une seule ligne `Outcome:`.
- [ ] Aucune branche vide n'est poussée sur session terminée, et une session terminée après commit conserve son travail.
