...
> **Correction à l'implémentation (2026-09-04) — le jeton retenu est `hold[review]`, pas `block[ac]`. L'intention de D5 est inchangée ; la prémisse factuelle sur laquelle elle reposait était fausse.**
>
> D5 justifiait `block[ac]` en affirmant qu'il « route vers l'opérateur **sans réessai automatique** ». Cette phrase venait de la note de grooming du ticket, et la revue de code l'a mise à l'épreuve du moteur. Mesure : `crates/mika-agent/src/server/verdict_handler.rs:908` (`handle_block_ac`) **dispatche structurellement un nouveau run claude-pilot de correction d'AC** — `try_engine_dispatch`, événement d'audit `ac_fix_dispatched` — jusqu'à `BLOCK_AC_MAX_RETRIES = 3` (`:44`), et n'escalade vers l'opérateur qu'**après** la limite. `handle_hold_review` (`:1564`) notifie l'opérateur et laisse la tâche `in_progress` : aucun dispatch, aucun réessai.
>
> Conséquence si l'on avait posé `block[ac]` : une PR de récupération dont la première ligne dit qu'elle n'existe pas pour être mergée aurait déclenché jusqu'à trois runs autonomes, consommant à chaque fois l'unique créneau de dispatch. Le correctif aurait fabriqué du travail de boucle à partir d'un diff vide — c'est-à-dire le contraire de ce que ce ticket protège.
>
> Second appui, indépendant : `block[ac]` porte le contrat de format de l'étape 2.5 (`PLAN-AC VERIFICATION` obligatoire, au moins un AC `❌`). L'item d'incident termine la revue **avant** l'étape 2.5, donc aucune ligne d'AC n'a été évaluée. Poser `block[ac]` là obligerait le modèle à omettre une section déclarée obligatoire, ou à inventer des verdicts d'AC qu'il n'a pas mesurés — précisément le mode de défaillance que R2 nomme. `hold[review]` est le verdict que ce même document désigne quand `PLAN-AC VERIFICATION` manque.
>
> **Ce qui est porté, et ce qui ne l'est pas.** L'intention d'AC5 — un refus **bloquant et non-approbateur**, dont le motif est que le diff ne peut satisfaire aucun AC — est intégralement portée : `hold[review]` est non-approbateur, il tient la PR, il notifie l'opérateur, et il rend l'approbation impossible. Ce qui tombe est la clause « sans réessai automatique » **attribuée à `block[ac]`** : elle décrivait un comportement que le moteur n'a pas. La corriger dans le plan plutôt que la laisser, c'est ne pas graver l'erreur de mesure dans l'artefact — même discipline que celle appliquée à F1 en première passe, dans l'autre sens.
>
> C'est un changement de jeton par rapport à ce que l'architecte a signé. Il est consigné ici, dans le corps de la PR et en commentaire du ticket, et reste renversable : si l'architecte tient `block[ac]` malgré la mesure, un seul mot change dans `qa-review/system_prompt.md` et dans l'assertion de contrat correspondante.
