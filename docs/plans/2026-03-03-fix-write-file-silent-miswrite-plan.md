...
# fix: write_file silently writes to wrong path when agent uses incorrect relative path

## Overview

The `write_file` tool silently writes files to wrong locations when the agent guesses an incorrect relative path. The tool reports success with the user-provided relative path, making it impossible for the agent to detect the mistake. The agent has no way to know its own home directory absolute path.
...
2. Agent's `home_dir` (base for `write_file`) is `~/.mika/agents/main/`
3. Agent called `write_file` with path `.mika/agents/main/identity.toml` — thinking its home was `~/` or `~/.mika/`
...
1. **Success message echoes the relative path, not the resolved absolute path.** The agent sees `"Wrote 86 bytes to '.mika/agents/main/identity.toml'"` and has no way to verify the actual write location. (`write_file.rs:120-121`)
...
### Change 1: Report resolved absolute path in write_file messages

In `write_file.rs`, change all user-facing messages to show the resolved absolute path instead of (or in addition to) the relative path. This gives the agent immediate feedback to verify correctness.

**Files:** `crates/mika-agent/src/tools/write_file.rs`
...
Apply the same pattern to ALL messages in write_file that reference the path:
...
**prompt.rs — write_file instruction (line 309-311):** Change from:
```
"- You can write files to your home directory with write_file. Paths are relative to your home. ..."
```
To (when home_dir is Some):
```
"- You can write files to your home directory with write_file. Your home directory is /home/user/.mika/agents/main/ — all paths are relative to this directory. For example, to write identity.toml at the root of your home, use path 'identity.toml'. If the file exists, you must review the current content and call again with confirm: true to overwrite."
```
...
- [x] `write_file` success message shows the resolved absolute path (e.g., `Wrote 86 bytes to '/home/user/.mika/agents/main/identity.toml'.`)
- [x] `write_file` confirmation/error messages show the resolved absolute path
- [x] System prompt includes the agent's home directory absolute path in the write_file instruction
- [x] `write_workspace` success/error messages show the resolved absolute path
- [x] All existing tests pass (`cargo test`)
- [x] New test: verify write_file success message contains the absolute home path, not just the relative input path
- [x] New test: verify system prompt contains the home_dir path when `home_dir` is `Some`

## Context

- PR #65 introduced the `write_file` tool (merged 2026-03-03)
...
- `write_file.rs:120-121` — success message format
- `prompt.rs:309-311` — write_file instruction in system prompt
