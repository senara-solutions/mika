...
- **Second-pass (same session, continuity preserved):** GROOMED. All five findings resolved with structural evidence. Two remaining uncertainties dispositioned: (1) `correction_message` continuous-text vs numbered-list — defer-to-implementation correct (registry orthogonality §6 wins; copy-edit toward `webhook_ready_label_dispatch`'s `(1)…(2)…(3)…` parallel form is no-cost at implementation time if eval shows compliance gap); (2) trigger-predicate substring — pinned in R1 against `format_callback_framing` at `agent.rs:109-113` (`msg.starts_with("A background task has")`) per the architect's pre-push tweak.
