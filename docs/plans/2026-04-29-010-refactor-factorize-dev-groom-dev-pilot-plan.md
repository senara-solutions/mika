...
# EXIT trap (it would be overwritten silently — bash trap is process-scoped, not function-scoped).
# Does NOT return — exec replaces the shell process.
dispatch_claude_pilot() {
    local ENTRY_COMMAND="$1"
    trap '_dispatch_lib_exit_trap' EXIT  # Install crash-callback fallback; once mika#887 ships, this trap also handles trace-tail.
