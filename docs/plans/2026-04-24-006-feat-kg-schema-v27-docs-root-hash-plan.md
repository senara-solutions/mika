...
- **Server callers:** `crates/mika-agent/src/server/mod.rs:787, 851, 973` — startup loop iterating `for (agent_name, agent_state) in &agents`. Passes the shared `docs_root.clone()` to each agent's KG constructors. After #738 lands, `docs_root` is sourced via `kg::config::resolve_kg_docs_root(&settings)`. **Post-#738-amendment:** the resolver returns a `(PathBuf, PathSource)` tuple (exposing source-of-origin for #778's per-agent policy classifier). #786's call site destructures the tuple and discards the source: `let (docs_root, _source) = resolve_kg_docs_root(&settings);`. #786 itself is indifferent to `PathSource` — the warn-and-skip downstream behavior applies uniformly. #778 is the consumer that branches on the variant.
