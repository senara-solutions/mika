...
## Open questions — second-pass resolutions (architect session `8288311f`)

1. **R4 same-status update — RESOLVED.** Architect: "use `update_task_metadata` to persist the HOLD note without a status transition, rather than `update_task_status(blocked)` which has wrong semantics (the child IS actively waiting for merge, not blocked on a dependency). This is a non-blocking implementation-time decision: the implementer tries the same-status write; if rejected, switches to metadata-only persistence. The HOLD semantic is in the note text, not the status value." → Phase 1 rewrite now names the `update_task_metadata` fallback inline.
2. **Test cohort placement — RATIFIED.** Architect NB2: "cohort by invariant (advance-on-completion) rather than by event source (callback vs webhook). The shared invariant is 'after a child completes, the milestone loop advances or halts with operator notification.'" → Phase 3 holds.
3. **`PostCallbackAdvance` race response — RATIFIED (block + notify).** Architect: "A `PostCallbackAdvance` backstop firing while a child is in HOLD means the merge is taking longer than the backstop timeout. That IS an anomaly the operator should see — slow merges with CI failures, merge conflicts, or GitHub outages are exactly the cases where operator attention is needed. Silent no-op would suppress a genuine signal." → Phase 1 idempotent rule holds as-is.
4. **M5 close-out R1 — RATIFIED (R1.a for this ticket; R1.c folds into mika#1218).** Architect: "R1.a is acceptable for this ticket. The asymmetry … is real but bounded: it only manifests when the LAST child of a milestone auto-merges. The operator types `continue` once. R1.c (`MilestoneCompletionCascade` SilentTrigger) is correctly scoped to mika#1218's engine-guard work where the structural milestone-advance machinery already lives." → Phase 2 step 5.5.c last-child branch + mika#1218 §Follow-ups hold.
