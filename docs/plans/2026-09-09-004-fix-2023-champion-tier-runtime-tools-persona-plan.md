...
- **Les tenants champion déjà provisionnés avant le 2026-08-28 — PORTE DE LANCEMENT BLOQUANTE, pas une dette.** `write_default_if_missing` (`home.rs:279`) ne réécrit jamais un `identity.toml` existant : un champion provisionné avant mika-cloud#209 **garde la surface opérateur sur disque**, et aucun changement de code ici ne l'en retire. C'est un geste d'exploitation (re-provision ou édition du disque), pas une ligne de Rust. **Verdict architecte (2026-09-09, session `05ad29c2`) : maintenu hors scope code — option 1 — SOUS RÉSERVE que cette vérification reste explicitement bloquante dans le processus de release.** La réserve est la condition du GROOMED ; l'écrire ici est ce qui l'empêche de s'évaporer.

  **Contrôle exigible avant d'ouvrir un compte champion à qui que ce soit** — pour chaque tenant de tier `champion` existant, lire l'`identity.toml` chargé dans le pod et vérifier l'absence de `shell-exec`, `tmux`, `git-ops` et `github`. Un seul tenant qui les porte encore rouvre le p0 *pour lui*, quel que soit l'état de ce ticket. Le fil note qu'Axelle était `champion`/`pending` le 2026-08-28 ; sa réactivation est précisément le moment où ce contrôle doit avoir été fait. Aucune ligne de code de ce plan ne l'exécute à ma place, et la garde de l'AC4 ne le détectera pas (cf. Fire-Disposition (b), angle mort nommé).
