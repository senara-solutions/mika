...
**Semantics of `Option<HeartbeatConfig>`:**
- `None` (no `[heartbeat]` section) → enabled (default behavior, backward-compatible)
- `Some(HeartbeatConfig { enabled: true })` → enabled (explicit opt-in)
...
This is simpler than the reflection accessor because heartbeat doesn't need timezone/time conversion — it always uses the fixed `0 0 * * * *` cron. The function returns `bool` rather than `Option<String>` since the cron expression is constant.
