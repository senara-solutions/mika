...
**Architect ruling (second-pass GROOMED, session `7cefb65c`): the attestation chain is the correct construction for slice 2** — the App key is RS256 (a signing key; repurposing it for RSA-OAEP encryption crosses a crypto-role boundary auditors flag), slice 2 has no secret-must-travel case (redaction guarantees it), and private-key distribution belongs in R7/slice 3. **Known limitation to carry forward (not a bug):** under the attestation chain the receiving pod has no private bundle-signing key of its own, so it **cannot re-sign its own exports in slice 2 — it is an import-only target.** This is consistent with R7 deferral; per-instance signing (the pod producing/signing its own bundles) is exactly slice 3's scope.
