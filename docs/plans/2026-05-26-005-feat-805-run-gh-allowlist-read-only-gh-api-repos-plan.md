...
1. `GH_ALLOWED_SUBCOMMANDS` includes `"api"` (line 1620).
...
const GH_API_READ_ALLOWED_PATTERNS: &[&str] = &[
    ...
    // Milestone management (read-only)
    r"^/?repos/[^/]+/[^/]+/milestones(/[0-9]+)?$",  // list or view milestone
];
```
...
Allowed patterns: repos/{owner}/{repo}/branches[/{branch}], repos/{owner}/{repo}/commits/{sha}, repos/{owner}/{repo}/milestones[/{number}].
```
...
fn validate_gh_api_scope(args: &[String]) -> Result<(), ToolOutput> {
    ...
    let matched = GH_API_READ_ALLOWED_PATTERNS.iter().any(|pattern| {
...
**Performance note:** The regex compilation happens per-call. For a tool invoked infrequently (gh api calls are rare), this is acceptable. If profiling shows it matters, use `LazyLock` / `OnceLock` to compile once. Start simple.
...
#[test]
fn test_gh_api_get_milestones_allowed() {
    let args: Vec<String> = vec!["api", "repos/senara-solutions/mika/milestones"]
        .into_iter().map(String::from).collect();
    assert!(validate_gh_api_scope(&args).is_ok());
}

#[test]
fn test_gh_api_get_milestone_by_number_allowed() {
    let args: Vec<String> = vec!["api", "repos/senara-solutions/mika/milestones/14"]
        .into_iter().map(String::from).collect();
    assert!(validate_gh_api_scope(&args).is_ok());
}
...
fn test_gh_api_patch_rejected() {
    let args: Vec<String> = vec!["api", "repos/o/r/milestones/1", "--method", "PATCH", "-f", "state=closed"]
        .into_iter().map(String::from).collect();
...
- **Regex performance:** Compiling N regexes per `gh api` call. Acceptable given rarity of `gh api` usage. Can optimize with `LazyLock` if needed.
- **Pattern too restrictive:** If future use cases need additional read-only endpoints (e.g., `repos/{owner}/{repo}/tags`, `repos/{owner}/{repo}/releases`), the allowlist is a single constant to extend. The error message guides the operator to the right place.
