...
**Base :** `main` @ `b7e12f73` (mesures antérieures : `ff02386f`, puis
`9aa39998`) — la PR compagnon #2441 (mika#2073) est mergée le 2026-09-22 10:20Z ;
...
supprimée au merge ; il est remplacé par celui-ci.

**Toutes les ancres de ce plan ont été re-vérifiées sur `b7e12f73`, et les six
lignes cibles ONT dérivé** (de +27 à +168 lignes) — c'est la seule dérive
load-bearing de cette révision, et elle porte précisément sur les lignes que le
corps du ticket et son AC3 citent. Les ancres de la garde, elles, n'ont pas bougé
d'une ligne. Deux rectifications de fond s'y ajoutent (§1.6 et §2.2bis). Détail
complet en fin de document.
...
Les six sites sont ceux que le ticket décrit, **mais leurs lignes ont dérivé
depuis la mesure du corps** (`fb8c108f`, re-vérifiée sur `ff02386f` puis
`9aa39998`). Mesuré sur `b7e12f73` :

| ligne (`b7e12f73`) | ligne au corps du ticket | item | `#[test]` ? | ce qu'il assère après le bootstrap |
|---|---|---|---|---|
| `:2279` | `:2252` | `test_provision_skips_existing_agent` | nu (attribut `:2272`) | écrase `soul.md`, assère la conservation |
| `:2300` | `:2273` | `test_provision_partial_state` | nu (attribut `:2293`) | existence de `mika-qa` |
| `:3378` | `:3210` | `pre_seed_identity` (helper, 12 appelants) | **pas un item de test** (`fn` à `:3377`, aucun attribut) | écrase `identity.toml` |
| `:3828` | `:3660` | `test_reconcile_config_toml_for_mika_dev` | nu (attribut `:3822`) | écrase `identity.toml` + `config.toml` |
| `:3849` | `:3681` | `test_reconcile_config_toml_idempotent` | nu (attribut `:3843`) | idem |
| `:3873` | `:3705` | `test_reconcile_config_skips_agents_without_spec_config` | nu (attribut `:3865`) | idem |

**Les numéros de l'AC3 sont ceux du corps du ticket, pas ceux de `main`.** L'AC3
est transcrite verbatim (section *Acceptance criteria*), et sa lettre — « les cinq
items de test », nommés par leur ligne — reste satisfaite : ce sont les mêmes cinq
items, aux mêmes noms. **L'invariant à vérifier est le nom du test, jamais son
numéro de ligne** ; un implémenteur qui chercherait `:2252` sur `main` ne
trouverait rien et conclurait à tort que le site a disparu. La preuve rouge de
l'étape 7 cite les lignes que le walker rend, donc celles de `main` au moment du
run — et le corps de PR doit dire les deux, sans quoi la sortie collée semble
contredire l'AC qu'elle atteste.
...
Le septième appel, `:954` (`:927` au corps du ticket), est dans `provision_agent`
(production, entouré de son `error!`/`continue`). Il **reste** un lecteur
d'environnement : c'est par lui que `MIKA_AGENT_TIER` atteint un agent bien connu
au premier démarrage (mika#1778). Le scan ne peut pas le voir de toute façon — il
n'est pas sous un attribut de test — mais la conversion ne doit pas le toucher non
plus. Un huitième hit de `bootstrap_agent` existe à `:886` : c'est un doc-comment,
il n'est ni un site ni une cible.

### 1.2 Les poseurs, mesurés sur tout le workspace

Trois `set_var("MIKA_AGENT_TIER", …)` hors `home.rs`, tous sériels :

- `crates/mika-agent/src/server/mod.rs:2066` — `agent_state_tier_survives_env_drift`, `#[serial_test::serial]`
...
- `crates/mika-cli/src/commands/agents.rs:2800` — `mika2230_the_env_is_the_fallback_when_the_flag_is_absent`, `#[serial_test::serial]`
...
`server/mod.rs:2069` et `:2077` — sont dans le test sériel ci-dessus. Tous les
autres appels du workspace (`mika-cli/commands/{setup,agents,ask,chat}.rs`,
`mika-agent/src/bin/mika-spirit.rs`, `tools/create_agent.rs`,
`server/mod.rs:632`/`:889`, `server/state.rs`) sont de production, hors de portée
du scan par construction.

### 1.3 Le périmètre élargi, mesuré

`crates/` porte **612** fichiers `.rs` sur `b7e12f73`. Le walker du garde,
...
§1.1 ; `pre_seed_identity` est invisible parce qu'il n'a pas d'attribut de test.

Le `+2` fichiers entre `9d3214e9` et `b7e12f73` est du bruit de `main`, pas un
signal : il ne change ni la population d'offenders (l'étape 7 la re-mesure) ni les
planchers du §2.3, qui sont dimensionnés à ~2/3 de la mesure précisément pour ne
pas avoir à suivre ce nombre commit par commit.

**Pas de `target/` sous `crates/`, vérifié.** `rust_sources_under` ne prune rien :
il descend tout répertoire et retient tout `.rs`. Un `crates/*/target/` le ferait
exploser en temps et en faux offenders (les sources générées portent des
`#[test]`). Mesuré sur `b7e12f73` : aucun, et le compte disque (612) égale le
compte de l'index (612), donc rien d'ignoré ne traîne. C'est une **précondition de
la mesure**, pas une garantie du walker — un artefact de build par crate la
casserait, et le §2.3 le verrait par un `files.len()` aberrant, non par un rouge
explicite.
...
2. **Le garde-fou « a-t-il lu ? »** est `scan.tests_seen >= 40` sur un seul
   fichier (`home.rs:2524`). Sur 612 fichiers ce nombre ne dit plus rien : un
...
Le workspace porte **2 106** items `#[tokio::test…]` et **35** attributs
`#[serial_test::serial]` (dont les trois poseurs du §1.2). Après U1 le scan les
...
### 1.6 `rust_sources_under` : trois appelants, tous intra-module

`source_guard::rust_sources_under(root)` (`source_guard.rs:932`) existe
précisément pour « la boucle que quinze gardes écrivent à l'identique ». La
révision précédente de ce plan le disait « à zéro appelant » — **c'est faux au
sens littéral et vrai au sens qui compte** : il en a trois, et tous les trois sont
dans `source_guard.rs` même (`test_only_modules:703`, `SourceGuard::files:986`,
`SourceGuard::for_each_under:1031`). Aucun appelant **hors de son module**. Ce
garde devient donc le premier consommateur externe ; on n'écrit pas une seizième
boucle. Le walker ne prune rien et ne trie que par nom — voir la précondition
`target/` du §1.3.

### 1.6bis Le piège : `for_each_under` ressemble au bon outil et est le mauvais

`SourceGuard::for_each_under(root, f)` (`source_guard.rs:1030`) appelle
`f(path, production)` pour chaque `.rs` sous `root`, avec en plus l'assertion
« le walk n'a rien vu » que le §2.3 doit écrire à la main. C'est **exactement** la
boucle voulue, à une chose près, et cette chose est éliminatoire : le second
argument est `self.production_of(&path)`, c'est-à-dire la **moitié production** du
fichier — `report_of_content` rend `blank_like(content)` pour un fichier test-only
et `mask_test_regions_report(content)` sinon. Les régions de test sont blanchies.

Un garde bâti dessus recevrait donc, pour chaque fichier, un source dont tous les
`#[test]` ont disparu : `tests_seen = 0` partout, zéro offender, **vert**. Le
garde serait structurellement aveugle à la seule population qu'il existe pour
lire. Les deux ancres du §2.3 l'attraperaient (c'est leur raison d'être, et ce
serait leur première prise réelle), mais un implémenteur qui ajusterait les
planchers pour « faire passer » aurait livré un détecteur mort.

**C'est écrit ici parce que c'est la simplification que la prochaine revue
proposera** — « pourquoi ne pas réutiliser l'helper qui fait déjà ça ? ». La
réponse tient en une phrase, et elle doit vivre **en commentaire au site du
garde**, pas seulement dans ce plan : *ce garde lit du code de test, donc il lui
faut la source brute ; `SourceGuard` masque le test par construction.* Un garde
dont le choix d'outil n'est pas justifié au site se fait « simplifier » un jour.
...
| assertion | plancher | mesure sur `b7e12f73` | ce que son rouge dit |
|---|---|---|---|
| `per_file["mika-common/src/home.rs"] >= 40` | conservé de mika#2073 | 67 `#[test]` | la racine a dérivé, ou `home.rs` a été renommé |
| `per_file["mika-agent/src/well_known_agents.rs"] >= 40` | même plancher | 77 `#[test]` | le walk n'a pas atteint le crate où vivent les six sites |
| `files.len() >= 400 && total.tests_seen >= 4000` | ~2/3 de la mesure | 612 / ~7 836 | la racine pointe sur un sous-arbre |

Ces trois assertions sont aussi le seul filet contre le piège du §1.6bis : un
source masqué rendrait les deux ancres à `0`. Leur message doit donc nommer les
**deux** causes possibles d'un rouge — racine dérivée **ou** source masqué — parce
que la seconde est invisible dans un chemin de fichier correct.
...
Pour `pre_seed_identity` (`:3378`), un doc-comment d'une ligne dit pourquoi le
...
(`:3378`) n'a pas d'attribut de test, donc le scan ne le voit ni avant ni après
la conversion : la preuve rouge attend **cinq** offenders, et le vert d'après
U2 en attend **zéro** — ni l'un ni l'autre ne dit quoi que ce soit de `:3378`.
Ce qui l'atteste est donc, et seulement : (a) le vert de
`cargo test -p mika-agent --lib`, qui exerce les douze appelants du helper ;
(b) la **revue de diff**, qui compte six conversions et un `:954` intact ;
...
2. Le garde walke `canonicalize(CARGO_MANIFEST_DIR/..)` via
   ...
   d'ancre (§2.3) avant les deux existantes. **Un commentaire au site dit
   pourquoi `rust_sources_under` et non `SourceGuard::for_each_under`** — le
   second masque les régions de test, donc rendrait ce garde aveugle à sa propre
   population (§1.6bis). Sans ce commentaire, la « simplification » est une
   question de temps.
...
7. **Preuve rouge** : `cargo test -p mika-common --lib mika2073 -- --nocapture`
   → attendu : `mika2073_the_guard_fires_on_a_relapse` vert,
   `mika2073_no_bare_test_reads_the_tier_from_the_environment` **rouge**,
   cinq offenders sous `mika-agent/src/well_known_agents.rs`, aux lignes que le
   walker rend sur la base du run (sur `b7e12f73` : `:2279`, `:2300`, `:3828`,
   `:3849`, `:3873` — et **non** les `:2252`/`:2273`/`:3660`/`:3681`/`:3705` du
   corps du ticket, cf. §1.1), `desyncs` vide, ancres satisfaites. Si le compte
   ...
   deux se distinguent à la ligne citée. Les cinq **noms de test** sont
   l'invariant ; les numéros sont ceux du commit.

   **Capture de la sortie : sous le worktree, jamais dans `/tmp`.** La
   permission-policy du pilote refuse toute écriture hors worktree (classe
   mika#2211) : un `tee /tmp/…` est refusé et la preuve est perdue au moment
   même où elle est produite. Rediriger sous le worktree (fichier temporaire à
   la racine, supprimé après recopie dans le corps de PR) ou lire la sortie
...
8. Les six appels du §1.1 → `bootstrap_agent_with_tier(…, AgentTier::Default)`
   (§2.5). `:954` (production) intact, `:886` (doc-comment) intact — le diff de ce
   fichier ne touche que le module `tests`. **Repérer les sites par leur nom de
   test, pas par leur numéro** (§1.1) : les lignes du corps du ticket sont
   périmées de +27 à +168.
...
12. Corps de PR : sortie de l'étape 7 **verbatim** (AC3), la phrase
    « `pre_seed_identity` est le corps d'un helper, hors de portée du scan par
    construction, converti au même geste », **et la table de correspondance des
    lignes** (§1.1). Sans elle, une sortie rouge citant `:2279` semble contredire
    un AC3 qui dit `:2252`, et le relecteur doit refaire la mesure pour s'en
    convaincre.
...
- [ ] Les six sites du §1.1 passent `AgentTier::Default` ; `:954` (production) est
      inchangé ; aucun `#[serial]` ajouté ni retiré.
...
- [ ] Le choix de `rust_sources_under` **plutôt que** `SourceGuard::for_each_under`
      est justifié en commentaire au site (§1.6bis) — sans quoi la prochaine revue
      « simplifie » vers un garde aveugle.
- [ ] La garde a été **vue rouge** sur les cinq items de test avant U2, sortie
      collée verbatim dans le corps de PR, `desyncs=0`, et la table de
      correspondance des lignes accompagne la sortie (§1.1).
- [ ] Le sixième site (`pre_seed_identity`, `:3378`) est attesté par les trois
      voies de la Fire-Disposition — vert `-p mika-agent --lib`, compte de six
      conversions au diff, doc-comment nommant l'angle mort — et par aucun
      détecteur.
...
**Les numéros de ligne ci-dessous sont ceux du corps du ticket et ne sont PAS
rafraîchis** — une transcription verbatim qu'on corrige n'est plus une
transcription. Ils sont périmés de +27 à +168 sur `b7e12f73` ; la correspondance
est au §1.1, et l'invariant vérifiable est le **nom** de chaque test.
...
- Le site de production `:954`, et les appels de production de `mika-cli`,
...
- **Tout carve-out de répertoire, `benches/` compris.** La mesure de 612 fichiers
  les inclut, « aucun filtre de répertoire » les couvre, et en exclure un serait
  la même allowlist déguisée que pour `tests/` (§1.3).
- **Porter la garde sur `SourceGuard`** (§1.6bis) : ce n'est pas une amélioration
  différée, c'est un chemin refusé — il masque le test.

## Risques

- **Un desync sur un des 612 fichiers.** Mesuré à zéro sur `9d3214e9` ; `main` a
  depuis avancé jusqu'à `b7e12f73` (+2 fichiers) sans toucher un fichier à raw
  string inhabituel, mais l'étape 7 le re-mesure. Si un desync
  ...
  exclure le fichier.
- **Dérive des lignes cibles, matérialisée entre deux grooms.** Les six lignes du
  corps ont bougé deux fois en trois jours (+27 à +168 entre `9aa39998` et
  `b7e12f73`, sur un fichier que la flotte édite quotidiennement). Ce n'est plus un
  risque hypothétique, c'est une mesure : l'implémenteur **doit** repérer par nom
  de test. Un patch construit sur les numéros du corps toucherait du code
  arbitraire — et `well_known_agents.rs` porte des constantes d'identité, donc le
  faux positif n'y est pas anodin.
- **Coût du garde.** 612 lectures + ~7 836 walks de corps par exécution de
...
- 2026-09-22 (re-groom sur `main` @ `b7e12f73`, par le chemin moteur dev-groom —
  commentaire opérateur du 14:12Z). **Cette fois les six lignes cibles ONT
  dérivé**, et c'est la seule dérive load-bearing : `:2252`→`:2279`,
  `:2273`→`:2300`, `:3210`→`:3378`, `:3660`→`:3828`, `:3681`→`:3849`,
  `:3705`→`:3873`, et le site de production `:927`→`:954`. Les natures sont
  re-vérifiées une à une (cinq `#[test]` nus avec leurs attributs, `fn
  pre_seed_identity` à `:3377` sans attribut, `:954` entouré de son
  `error!`/`continue`, `:886` doc-comment). Les ancres de la garde sont
  **inchangées à la ligne** : `home.rs:404`/`:2389`/`:2473`/`:2513`/`:2524`/`:2548`,
  `source_guard.rs:932`, `CLAUDE.md:22`. Descriptifs rafraîchis : fichiers
  611→612, `#[test]` de `well_known_agents.rs` 76→77, `#[tokio::test` 2 093→2 106,
  `#[serial_test::serial` 32→35, poseurs `mod.rs:1998`→`:2066` et
  `agents.rs:2143`→`:2800`, lecteurs voisins `:2001`/`:2009`→`:2069`/`:2077`.
  **Trois rectifications de fond**, toutes trouvées en re-lisant le code plutôt
  qu'en re-lisant le plan : (a) §1.6 — `rust_sources_under` a **trois** appelants,
  tous intra-module, et non « zéro » ; l'affirmation utile (« aucun hors de son
  module ») est conservée et corrigée ; (b) **§1.6bis, nouveau** —
  `SourceGuard::for_each_under` est la boucle exacte qu'on veut écrire et un
  piège : il passe `production_of`, qui blanchit les régions de test, donc un garde
  bâti dessus verrait `tests_seen = 0` et serait vert en étant aveugle ; le refus
  est porté en Hors périmètre, en DoD et en commentaire au site ; (c) étape 7 —
  le `tee /tmp/…` prescrit est **refusé par la permission-policy du pilote**
  (classe mika#2211), ce qui perdrait la preuve d'AC3 au moment de la produire.
  Ajouts : précondition « pas de `target/` sous `crates/` » (§1.3), risque de
  dérive des lignes promu de l'hypothèse à la mesure, et obligation de joindre la
  table de correspondance à la sortie rouge (sans quoi elle semble contredire
  l'AC3 qu'elle atteste).
- **État de la branche à la reprise, nommé plutôt que tu.** Le worktree local
  porte deux commits d'implémentation antérieurs (`03c3182e` la conversion +
  l'élargissement, `a322ff7a` un sauvetage `wip`), reliquats du cycle dont la PR
  #2483 a été fermée et la branche distante supprimée (commentaire 14:12Z : le
  produit est né d'un contournement de la porte `dispatch-grooming-verified`, donc
  rejeté). Ce groom ne touche pas cet historique — la réconciliation de branche
  appartient à `_set_up_worktree`, et réécrire l'historique n'est pas dans le
  périmètre d'un groom content-only. **Conséquence à connaître avant
  d'implémenter : l'arbre de travail peut déjà contenir tout ou partie du
  livrable.** L'implémenteur doit d'abord établir sa base (`git log
  origin/main..HEAD`) et, si ces commits sont encore là, décider avec l'opérateur
  s'il repart de `main` — la preuve rouge de l'étape 7 est **inobtenable** sur un
  arbre où la conversion est déjà faite, et c'est ce qui rend la question
  bloquante plutôt que cosmétique.
