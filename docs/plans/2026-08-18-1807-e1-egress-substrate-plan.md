...
**Résolution v1 (sami-tranchée 2026-08-18) : `tracing` structuré + Prometheus metrics agrégés, ZÉRO tenant_hash, ZÉRO query content.**
...
Bucket-64 avait été proposé pour cohort SRE trouble-shooting, mais sami a arbitré strip total en v1 :
...
**Ce discipline transporte** : la Prime rule "construis l'incapacité, ne promets pas la retenue" (analogie #1796). On construit un log path qui NE PEUT PAS écrire un tenant identifier — pas seulement une promise que le code ne le fait pas.
