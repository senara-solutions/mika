...
### Brique −1 — la mesure qui ne coûte rien, et qui passe en premier

Avant toute écriture de code, trois lectures. Elles tiennent en quelques minutes, elles n'exigent aucun déploiement, et elles peuvent **clore H3 à elles seules** (M9, M10) :
...
Si ces trois lectures donnent une réponse nette, **le reste du plan change de forme** : H3 confirmé ne demande ni brique 0 ni recalibrage, mais un geste d'opérateur et une garde contre la récidive. Ce n'est pas une raison de sauter la brique 0 — le trou d'observabilité resterait ouvert pour le réglage suivant — mais c'en est une de ne pas la traiter comme le premier geste.
...
- **AC0 — les trois lectures préalables (opérateur, avant tout code).** Tir sur : la brique −1. Disposition : **halt-and-surface**. Si le `config.toml` de mika-arch ne porte pas `llm_http_timeout_secs`, ou si aucun `config_reconcile` n'apparaît pour lui au dernier démarrage, alors **H3 est confirmé et le reste du plan est suspendu** : la cause est un provisionnement gelé, le remède est un geste d'opérateur, et ni la brique 2 ni aucune modification de valeur ne s'appliquent. Écrire la mesure dans mika#2293 avant de continuer.
...
- Les trois lectures de la brique −1 sont faites et consignées **avant toute écriture de code** (AC0). Si elles confirment H3, le reste de cette liste est suspendu et le ticket repart sur le provisionnement.
...
0. **AC0 — les trois lectures préalables sont faites et consignées.** Le contenu du `config.toml` de mika-arch, la présence ou l'absence de `config_reconcile` pour lui au dernier démarrage, et l'état des variables d'environnement pertinentes sont relevés **avant toute écriture de code** et reportés dans le PR ou dans mika#2293. H3 confirmé suspend le reste.
1. **AC1 — le budget effectif est observable.** Un événement INFO `llm_budget_resolved` est émis à la construction de chaque provider, inconditionnellement, portant `agent_id`, `http_timeout_secs`, `agent_total_timeout_secs`, `max_attempts`, `worst_case_failure_secs`, et la **provenance** de chacune des deux valeurs parmi `default` / `agent_config` / `process_env` / `agent_dotenv` — `default` devant rester distinguable du repli hors-cascade décrit en M11, et la reconstruction devant respecter l'ordre réel de la cascade (`.env` per-agent au-dessus de l'environnement du process, mika#2218).
...
|---|---|---|
| AC0 | Brique −1 | Trois lectures opérateur, sans code — halte si H3 (AC0 Fire-Disposition) |
...
- **Avant tout code — les trois lectures de la brique −1 (AC0) :**
...
- **AC0 confirme H3** (pas de clé dans le `config.toml` d'arch, ou aucun `config_reconcile` pour lui) → halte immédiate, **avant toute écriture de code** : la cause est un provisionnement gelé derrière `MIKA_DISABLE_AGENT_PROVISIONING` ou `dev_mode`, pas un budget mal calibré. Le remède est un geste d'opérateur ; le recalibrage ne s'applique pas ; la brique 0 reste souhaitable pour le réglage suivant mais cesse d'être le chemin critique.
