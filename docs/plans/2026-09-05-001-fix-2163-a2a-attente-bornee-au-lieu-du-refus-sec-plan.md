...
collision protocolaire avec un pair A2A tiers. **Retenu : `AGENT_BUSY = -32099`.**

Le raisonnement, parce que le choix du nombre n'est pas arbitraire : JSON-RPC 2.0 réserve
-32000..-32099 aux erreurs serveur définies par l'implémentation ; la spec A2A s'y taille une plage
**en numérotant depuis -32001 vers le bas** (-32001 `TaskNotFound` … -32006 `InvalidAgentResponse`,
-32007 `AuthenticatedExtendedCardNotConfigured`). Prendre `-32010` reviendrait à camper trois places
devant une numérotation qui avance. `-32099` est l'extrémité opposée de la même plage réservée :
la collision exigerait que la spec A2A publie 92 codes de plus. `from_code` reçoit son libellé,
`"Agent is busy"`.

**Dette de compatibilité, nommée et datée.** Le dépôt implémente A2A **v0.3**
(`crates/mika-a2a/CLAUDE.md:3`) et ne vendorise pas la spec ; la plage assignée n'est donc pas
vérifiable hors ligne depuis ce plan. **Avant merge**, l'implémenteur confronte `-32099` à la spec
A2A v0.3 publiée. Si le code s'avère pris ou réservé, il descend dans la plage libre la plus haute —
la conception ne change pas, seul le nombre change. T7 fixe la non-collision **locale** ; il ne peut
pas fixer la non-collision **protocolaire**, et ne prétend pas le faire.
...
| T3 | AC2 | File pleine (profondeur 1, N+1 attendants) → `-32099`, `data.reason = "queue_full"`, `retry_after_ms` présent | idem |
| T4 | AC2 | Délai dépassé (`wait_timeout_ms` court, verrou jamais rendu) → `-32099`, `data.reason = "wait_timeout"` | idem |
...
| T8 | AC7 | `message/stream` : le client coupe pendant l'attente du verrou → `tx.closed()` gagne le `select!`, la tâche rend son permis et **n'acquiert jamais** `agent_lock` ; l'agent n'est jamais lancé (§3.6). Vérification : verrou tenu par une tâche tierce, abonné SSE droppé, puis assertion que le compteur d'appels de l'agent reste à zéro **et** que le permis est rendu (profondeur de file revenue au plein) | `a2a_queue_contention.rs` |
...
| AC2 — borne et saturation explicites, code qui dit la contention | §3.2, §3.3 ; T3, T4 | `-32099` et non `-32007` (assigné par la spec A2A) |
...
| AC7 — pas de tour orphelin sur le chemin streaming | §3.6 ; T8 | Le délai de détection est borné par le keep-alive SSE ; T8 le mesure |
