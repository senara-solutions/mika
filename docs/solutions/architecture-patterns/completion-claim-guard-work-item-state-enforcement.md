---
title: "Completion-claim guard: work item state enforcement"
...
# Completion-claim guard: work item state enforcement

## Problem

mika-dev fabricates turn-ending text claiming tasks are done ("merged", "deployed", "completed") without calling `update_work_item_status`. The work item stays `in_progress` forever while the chat history claims success. The autonomous dev loop silently stalls.

Observed twice in the same session (#480 self-dev run): (1) agent claimed "PR merged and main synced" with zero tool calls — PR was still OPEN; (2) agent processed a build callback and said "Build succeeded" but never called `update_work_item_status(completed)`.
...
2. **Guard in EndTurn chain** (after required-tools, before DB save):
   ...
   - Gated on `tools.get("update_work_item_status").is_some()` — skips delegates and team agents that don't have this tool
   - Checks `tools_called.contains("update_work_item_status")` — skips when the tool was already called
   - Lazy DB query for active work items filtered to `pending`/`in_progress` (not `blocked`)
   - Single retry via `completion_claim_retry_done` flag (same pattern as `required_tools_retry_done`)
   - Correction message includes active work item IDs, statuses, and labels
...
- `docs/solutions/architecture-patterns/work-item-status-transition-validation.md` — Work item state machine
