...
**Residual coverage (post-#894 adversarial review).** Three escape shapes remained after the #894 extension — surfaced by adversarial code review on the #894 PR and filed as separate follow-ups, not silently absorbed: (a) descriptor-word absorption — `the gh_read tool is not available` (P2 captures `tool`, not `gh_read`; mika#1177), (b) antonym `unavailable` — not in P2's terminal alternation (mika#1177), (c) modal/periphrastic negation like `unable to call gh_read` (mika#1177). Composition gap: the `has_successful_pr_review` early-accept path previously skipped the asserted-unavailability guard (6c) and the assert-grounded guard (6d) alongside completion-claim and action-claim — fixed by mika#1178 (guards 6c and 6d now fire regardless of `skip_remaining_guards`). The three remaining shapes (a/b/c above) are low-severity (required-tools-gate is the downstream safety net) but real — claiming the guard "closes" the failure family today would mislead a future reader citing this doc. Coverage is mature, not exhaustive.
...
- mika#1177, mika#1178 (2026-05-17) — residual escape shapes surfaced by adversarial review on the #894 PR: descriptor-word absorption + antonym `unavailable` + modal negation (#1177); has_successful_pr_review skip-path composition gap (#1178). #1178 fixed: the `has_successful_pr_review` skip path no longer bypasses the asserted_unavailability guard (6c) or the assert-grounded guard (6d). #1177 remains tracked; required-tools-gate is the safety net.
