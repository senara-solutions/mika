...
*Structural counterpart:* a new "asserted-unavailability guard" in the EndTurn chain — keyword detection on assistant text for unavailability claims about tools that are in fact in the active tool registry, single retry with corrective re-prompt. Same shape as the existing three guards. Guard must reconcile against the *enabled* tool registry, not the full registry, to avoid false-firing on legitimately disabled tools (e.g. `MIKA_ARCH_DISABLED_TOOLS` evictions — saying "X is not available" when X is genuinely denylisted is a true statement, not a fabrication). Tracked separately; not in scope for this doc.
...
The required-tools-gate fires *after* the verdict is shaped (current chain: text-tool-call → required-tools → completion-claim). Both Rule 1 and Rule 2's structural counterparts above call for earlier-firing guards — a tool-precondition guard for Rule 1's quoted-resource shape, an asserted-unavailability guard for Rule 2's claim shape. Both are tracked as separate engine tickets, not in scope for this doc. When either lands, the citation in core memory pointing at this doc gets updated to reference the structural mechanism that supersedes the prompt-level rule.
