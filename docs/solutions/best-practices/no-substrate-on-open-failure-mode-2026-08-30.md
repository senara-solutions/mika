...
3. **A mitigation is only active once measured — and the measurement must be taken on the population the mitigation touched.** A model swap, a config flip, a version pin — all of them are *claims* until a measurement taken after the change shows the failure mode gone. mika-dev was swapped from glm-5.2 to glm-5.3 on 2026-08-26; the 2026-08-29 measurement still found 102 of 120 sessions silent. Closing #1910 on the strength of the swap would have recorded a repair that the next measurement contradicts.

   **Corrected 2026-09-20 (mika#1950), and the correction sharpens the rule rather than weakening it.** Those two sentences sit side by side, which makes the second read as a test of the first. It is not. Re-measured session by session, **all 120 of those sessions ran under `claude-opus-4-8[1m]` and none under any GLM**: `claude-pilot` is the *implementation* surface and runs on Claude, while `mika-dev` — the *dispatcher* — is what carries GLM. The 2026-08-29 figure is a true measurement of pilot-cycle emptiness and says nothing about glm-5.2 or glm-5.3 either way. The same re-measurement found the mika#1910 symptom proper (the `[guardrail] empty_response:` event) **0 times in 2353 sessions**, and found `policy:deny` — the long-standing suspect — present in 50.6% of empty sessions against 50.0% of productive ones, i.e. not discriminating at all. So the rule stands and gains its sharpest illustration: a measurement taken on the wrong population is not a weak mitigation check, it is not one. Data, script and halts: [`docs/eval/mika-1950/README.md`](../../eval/mika-1950/README.md).
