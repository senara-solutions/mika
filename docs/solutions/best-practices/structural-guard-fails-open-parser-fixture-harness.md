...
applies_when:
  ...
  - "Loosening a guard's parser so it accepts more syntax (traversing lines it used to refuse)"
  - "Writing a 'lock' test meant to redden if a guard branch is later removed"
related_components:
  - testing_framework
...
**Loosening the parser is a guard change: ask the fail-open question again.** A fail-closed parser eventually refuses legitimate code, and someone relaxes it. The relaxation is where a new fail-open hides, because it turns lines the parser used to *refuse* into lines it now *skips*. mika#2054 (second pass) taught `production_lines()` in `scripts/verify-egress-no-log.sh` to traverse a blank line, a `//` comment or another attribute between `#[cfg(test)]` and its item — three idiomatic shapes that had been failing CI with a false diagnostic. The plan, two architect passes and the loop's QA review (APPROVED twice on the rescued PR #2546) all read it as a pure false-positive fix. `/ce:review` then reproduced a new fail-open it had opened: a traversed line can *itself carry the item*. `#[cfg(test)] use crate::testing::helper;` (attribute and item on one line), a blank line, then a production `mod client { … warn!("leak") … }`; or `#[cfg(test)]` then `#[allow(unused_imports)] use x;` then `mod client {`. In both, the item line was swallowed, the pending state stayed armed, and the next meaningful line — the production module — was taken as the test item and skipped: exit 0, "No egress no-log violations found", where the pre-relaxation parser reported the violation. The rule that closed it: **a traversal may only skip lines that provably carry no item.** Test it on the sanitized line, not the raw one — an attribute is traversed only when it is alone on its line (`sanitize($0) ~ /^[[:space:]]*#\[[^]]*\][[:space:]]*$/`, `scripts/verify-egress-no-log.sh:285`), and a `#[cfg(test)]` followed by anything on its own line fails closed on that line instead of arming the state (`:241`). Harness cases V14 and V15 (`scripts/test-verify-egress-no-log.sh:557`) were seen red against the relaxed parser before the fix.

The same review found the relaxation's companion claim over-promised. The new comment said an unbalanced brace count "fails closed at end-of-file". It does — for *one* miscount. Two opposite miscounts cancel: a `'{'` char literal in one test module and a `'}'` in a later one leave the depth balanced at EOF while every production line between them was skipped. Measured: exit 0 on the relaxed parser (and on the pre-relaxation one — the hole predated the change, the comment made it a false guarantee). Closed twice over: `sanitize()` now drops simple char literals (`:146-147`), and a `#[cfg(test)]` seen while an inline test module is still counted open fails closed with exit 4 (`:208-219`), which also catches cancel-outs the sanitizer cannot model (block comments). Cases V16–V17 (`:601`).

**A lock test is only verified by deleting what it locks.** A test that is green on both sides of the fix — a "lock" whose value is to redden if a branch is later removed — cannot be validated by running it; it passes by construction. Validate it by mutation: remove the branch it claims to pin and watch it fail. Two locks in the same change failed that check. V3b claimed to pin the stacked-`cfg` refusal, but asserted only the exit code and the substring `egress-no-log, parser`, which *every* parser failure emits; deleting the stacked-`cfg` branch sent the fixture into the generic unmodeled-form branch and the harness stayed at 36/36. It now asserts the branch's own diagnostic, `` stacked `cfg` attribute ``. V13 claimed "both awk parsers call `sanitize()`" by counting call sites globally (≥ 2); `production_lines()` alone has two, so replacing `macro_block()`'s call with `s = $0` stayed green. It now counts per function body (`scripts/test-verify-egress-no-log.sh:541-545`). The pattern: **assert the text only the guarded branch can produce, and scope a population count to the unit it claims to cover.**

All four were found by one review round whose reviewers built the mutant and pasted the exit code, then confirmed by an independent validator with its own reproduction — the "reproduce, don't argue" rule above, paying out again. None was visible to plan review, architect review or a QA pass that ran the (green) harness: for a guard, *approved* and *fail-open-free* are different claims, and only the adversarial/mutation lens tests the second.
...
- `scripts/verify-egress-no-log.sh` — its `production_lines` awk function models Rust `#[cfg(test)]` scoping. It recognised `mod NAME;` and `mod NAME {`; for *any other* form after the attribute — a bare `#[cfg(test)] fn`, a `#[cfg(test)] impl` — it called awk `exit`, silently dropping the remainder of the file from the scan. The comment beside it said the case was "worth a manual lint revisit" — a promise, in a file whose header preaches construction over promise. It was the same class as mika#2039's parser. **Fixed in mika#2054**: the `exit` branch now prints a diagnostic naming the offending line and what to add to the parser, then exits non-zero so the guard fails closed rather than abandoning the file; a legitimate `#[cfg(test)] mod { … }` block is still modelled and skipped. The second, milder instance in the same guard — the `info!` allowlist read a fixed 8-line window after the call, so an `info!` with a non-allowlisted event passed if an allowlisted `event = "…"` token happened to sit within 8 lines below it — was fixed in the same ticket: the check now attaches to the macro's own parenthesised call block and fails closed when the block cannot be delimited. Its negative behaviour is now pinned by `scripts/test-verify-egress-no-log.sh` (anti-vacuity: a violation placed *after* an unmodeled `#[cfg(test)]` is caught), wired into CI's `egress-no-log-lint` job. A second pass (mika#2054, PR pending) relaxed the parser to traverse intercalary lines and counted braces on sanitized text — and the relaxation opened its own fail-open, described under "Loosening the parser" above. The file is the worked example for both halves: refusing what you cannot model, and not skipping what you have not proven empty.
