...
The mika-gateway Rust crate lived in a separate repository
with a git SSH dependency on mika-common. It needed to
move into the mika monorepo alongside mika-agent and mika-cli.
...
6. **Stale documentation** — 3+ docs still referenced gateway in the old repository
...
```bash
cp -r ../mika-gateway/ mika/crates/mika-gateway/
```
