...
The **google-workspace** skill provides `run_gws` for interacting with Google Workspace (Gmail, Calendar, Drive) via the `gws` CLI. It uses a service allowlist (`gmail`, `calendar`, `drive`) and blocks credential flags (`--token`, `--credentials-file`, `--config`, `--config-dir` including `--flag=value` forms). Scrubs all `GOOGLE_WORKSPACE_CLI_*` and `MIKA_*` env vars from child processes, injecting only the configured `MIKA_GOOGLE_TOKEN` as `GOOGLE_WORKSPACE_CLI_TOKEN`. Requires `gws` CLI installed (included in Docker image). `timeout_secs = 45` to accommodate first-call API schema discovery.
