...
### Bundled-Skill Re-sync

`mika-server` rewrites `~/.mika/agents/<agent>/skills/<skill>/*` from `BUNDLED_SKILL_MANIFESTS` (generated by `crates/mika-agent/build.rs` from `skills/bundled/`) on every agent session start. The mechanism is controlled by `MIKA_DISABLE_BUNDLED_SKILLS` (default `false`; CLAUDE.md warns against enabling in production because it prevents security updates to handler scripts). Hot-edits to deployed skill files at `~/.mika/agents/<agent>/skills/<skill>/system_prompt.md` (and other bundled artifacts) are overwritten on the next session start; durable changes must go through the source tree at `skills/bundled/<skill>/` and propagate via `make deploy`. The literal-string allow-list pattern in `permission-policy/system_prompt.md` is tracked for redesign at [mika#935](https://github.com/senara-solutions/mika/issues/935).
