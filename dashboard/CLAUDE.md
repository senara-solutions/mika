...
- **Shared UI components:** `@samidarko/ui` extracted into `packages/ui/` (npm workspace)
...
Event Timeline (`/timeline`), Agents, Sessions, Traces, Tasks (+ detail), Team Runs, LLM Calls (+ detail), Tool Calls (+ detail), Dev Runs (+ detail). Auth via `VITE_MIKA_DASHBOARD_TOKEN` env var. Bearer token. Imports shared components from `@samidarko/ui`. All list pages expose `<TimeRangeFilter />` with URL-reflected `?from=...&to=...` state (mika#659). Time-sensitive pages expose `<LiveRefreshToggle />` with `useLiveRefresh()` hook for consistent auto-refresh (mika#662).

**URL state (mika#664):** All filter, pagination, and tab state is URL-reflected for shareable links and browser back/forward support. The central hook `useSearchParamsFilter` (in `dashboard/src/hooks/`) manages URL query params. Filter changes auto-reset all page params via `ALL_PAGE_PARAMS`. Tasks page uses section-prefixed page params (`wi_page`, `trt_page`, `cb_page`, `sched_page`) for independent section pagination. Agents page search commits to `?search=` on Enter. SessionDetail tabs use URL path segments (`/sessions/:id/llm-calls`) with type-guarded validation and default-tab omission (mika#676). AgentDetail tabs use `?tab=` with type-guarded validation and default-tab omission.

**LLM Calls page (mika#660, mika#672):** `<CostTrendChart>` time-series visualization above the table, showing estimated cost over time aggregated from `llm_calls` token counts. Two variants: total (single line) and stacked-by-agent. Uses recharts. Backed by `GET /api/v1/llm-calls/cost-trend` with auto-bucketing (hourly <3d, daily ≥3d). Server defaults to last 24h when no `from` param. Pricing is estimated server-side via `crates/mika-agent/src/pricing.rs`. **Expandable body viewer (mika#672):** Rows with `response_text` or `reasoning` content use `<ListRow variant="expandable">` with inline `<LlmBodyViewer>` (in `dashboard/src/components/`). Bodies lazy-load via `useLlmCall(id)` on expand (list endpoint returns `has_response_text`/`has_reasoning` boolean indicators, not body content). JSON payloads auto-detected and pretty-printed. Bodies >10K chars show truncation indicator with "Show all" toggle. Rows without bodies render as `<ListRow variant="static">`.

**Live-refresh (mika#662):** Standardized auto-refresh across time-sensitive pages via `<LiveRefreshToggle />` (from `@samidarko/ui`) + `useLiveRefresh()` hook (in `dashboard/src/hooks/`). Detail pages (Dev Run, Team Run) auto-refresh at 5s when the run is active (non-terminal status), with toggle to pause. List pages (Sessions, Tasks, LLM Calls) have user-toggleable 15s polling (off by default). Polling suppressed when filters are active or user navigates past page 1 (`isDefaultView` guard). Tab visibility pause is free via React Query's `refetchIntervalInBackground: false` default. Event Timeline refactored to use same primitives. Home page unchanged (always-on 15s, no toggle).
...
**Cost visibility (mika#667):** Cost is surfaced as a first-class signal across four dashboard surfaces via `<CostMeter>` (from `@samidarko/ui`): (1) **Dev Run detail** — full-variant meter in the stats row showing per-run cost with threshold colors. (2) **LLM Calls list** — chip-variant meter in a dedicated cost column, computed server-side from token counts × provider pricing via `crates/mika-agent/src/pricing.rs`. (3) **Agent detail** — chip-variant meter in the header showing today's cost (24h rolling aggregate). (4) **Home overview** — `CostSummaryWidget` with `CostTrendChart` showing 24h cost trend (shipped via mika#666). The `cost_usd` field on LLM call API responses is computed server-side by `enrich_llm_calls_with_cost()` using the existing pricing module.

## `packages/ui/`

`@samidarko/ui` shared React component library (Vite library mode, published to GitHub Packages). Components: StatusBadge (six-variant: success/warning/error/info/neutral/blocked), TaskStatusBadge (thin adapter delegating to StatusBadge), Pagination, EmptyState (with optional action affordance), LoadingState (list/detail skeleton variants with ARIA), ErrorState (retry + details affordances with ARIA), CopyButton, MarkdownContent, ListRow (three-variant: static/navigable/expandable — canonical row primitive for list/table surfaces with keyboard a11y and ARIA), SelectFilter (categorical one-of-N filter dropdown), AgentFilter (thin adapter delegating to SelectFilter with consumer-injected agents prop), TimeRangeFilter (presets + custom picker, ISO 8601 emission, server-side enforcement), TokenBudgetBar (three-tier color threshold progress bar with ARIA meter semantics), CostMeter (unbounded threshold-based cost display with ARIA status semantics — two variants: full/chip), LiveRefreshToggle (auto-refresh toggle switch + LIVE badge composition — canonical affordance for all live-refresh surfaces). Utils: formatTime, badges, agentColors, formatApiError (human-shaped error message conversion). Theme CSS with design tokens (colors, spacing scale). Peer deps: React 19, Tailwind CSS v4, lucide-react. See `packages/ui/CLAUDE.md` for the enforcement rules and canonical primitives table.
...
The dashboard is audited for WCAG 2.1 AA compliance. See `docs/audits/2026-05-06-dashboard-a11y-audit.md` for the initial audit and finding catalog. Primitive-level a11y (ARIA, keyboard, focus) is enforced via axe-core tests in `packages/ui/`. Page-specific a11y findings from the audit are filed as follow-up issues.
