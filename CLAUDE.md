...
- `MIKA_STUCK_PENDING_REAPER_GRACE_SECS` — Grace window (seconds) before the stuck-pending reaper acts on a `pending` `self_dev` **issue** parent that no callback child represents any more (default: 2700, 45 min; mika#2045). The nominal `pending → failed` transition was measured at 17–25 min, so 45 min sits at 1.8x the top of that window and a merely slow task is never touched. Invalid/≤0 values fall back to the default (WARN-logged). Shared with the `mika tasks stuck` probe so operator and engine report on one population.
  - **The verdict is re-checked against the world, not against the scan (mika#2185).** The age window decides who becomes a *candidate*; it never decides the outcome on its own. Before acting, the reaper re-poses **both** clauses of `find_orphaned_pending_issue_tasks` — no live deferred wrapper, and no active non-deferred callback — on the inventory it has already read, so the audit line and the verdict describe one moment. Fail-**closed** on either: an unreadable answer passes the turn, because doing nothing costs 60 s and acting on an unchecked premise costs a parent. Greps in `$MIKA_SPIRIT_LOG_FILE`: `stuck_pending_skipped_wrapper_appeared` and `stuck_pending_skipped_dispatch_in_flight` (INFO — **non-empty under contention is the healthy shape**; each line is a window closed on a healthy parent, i.e. the measure that the guard bites), `stuck_pending_inventory_unreadable` and `stuck_pending_active_callback_unreadable` (WARN — **expected zero**; each line is a parent the reaper can neither repair nor expire, and the remedy is to repair the read, never to loosen the guard). Two distinct names per pair because the two axes call for different queries and must stay countable apart. Full contract, the three opposed fail-safe directions, and what the guard does *not* close: `crates/mika-agent/CLAUDE.md` § *The stuck-pending verdict is re-checked against the world*.
