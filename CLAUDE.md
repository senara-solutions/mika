...
- `MIKA_CALLBACK_DELIVERY_BACKOFF_BASE_SECS` / `MIKA_CALLBACK_DELIVERY_BACKOFF_MAX_SECS` — the backoff, `base × 2^(attempts−1)` capped at max (defaults `60` / `3600`). The base is one scan interval, so a single transient failure retries on the next scan and the nominal path is unchanged. Both are clamped to a floor of 1, and max is never allowed below base — a `0` base would make every doubling `0` and write a `next_fire_at` in the past, i.e. a backoff visible in the row that does nothing.
