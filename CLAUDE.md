...
- `cargo test` — Run all tests (~872 tests)
...
- **Long-running exec handlers:** `ToolHandler::Exec` supports `long_running: bool` and `estimated_duration_secs: Option<u64>` fields in `skill.toml`. When `long_running: true` and a `LongRunningContext` is provided (conversation mode only, not silent/team), the executor creates a callback task, injects `__mika_task_id` and `__mika_agent` env vars into the subprocess, spawns the process with `kill_on_drop(false)`, and returns immediately with a "task created" message. A background monitor (`spawn_long_running_exec`) awaits the child process: on success it completes the callback task with stdout, on failure it marks it failed. The subprocess PID is recorded. Team agents don't use long_running directly — the team engine detects pending grandchild callback tasks and suspends/resumes the run.
