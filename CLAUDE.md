...
- `MIKA_PILOT_STALL_REAP_AGE_SECONDS` — silence window in seconds before a live pilot whose worktree has received no write is reaped (default `2700`; `0` → default with a WARN). **The default is bounded on both sides by measurement, not chosen for roundness.** Below: a pilot whose internal watchdog *runs* cannot stay silent past **1800 s**, so the class this reaper exists to see starts above it. Above: the shorter of the two founding stalls is **58 min = 3480 s**, so a threshold at or beyond 3480 s misses that case outright — which is why the architect's prudential suggestion of 3600–5400 s was declined on measurement. The usable window is `]1800 s, 3480 s[` and 2700 s sits near its middle. Negative control it must clear: the **healthy** run c3f9a2f9 (mika#2238) wrote its worktree at 20:19 and again at 20:43 — a **24-minute** inter-write gap on a pilot that was working, which the ticket body's illustrative 15–20 min would have killed.
...
- **Operator surfaces.** SQL: `SELECT COUNT(*) FROM audit_events WHERE tool_name = 'pilot_silent_stall'` — that count IS the flip condition above; each row carries the measured idle age, the worktree path, `disposition_armed` and `transitioned` in its `reasoning`, and `after_value` states what actually happened (`in_progress` when observing or when the kill failed, `failed` when it fired) rather than what was intended. Grep: `pilot_silent_stall` (WARN) in `$MIKA_SPIRIT_LOG_FILE`, and `dispatch_worktree_declared` / `dispatch_worktree_declare_failed` on the shell side — a sustained `declare_failed` means dispatches are running unwatched.
