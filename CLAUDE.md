...
- **What this predicate does NOT own:** the `Branch` and `Plan` callout conditions, which stay with their two callers and stay divergent (`auto_pull` requires the `` `docs/plans/ `` prefix, `executor` accepts a bare `docs/plans/`). Unifying that half **is** mika#2120, under operator arbitration. `grooming_marker::tests::mika2120_divergence_is_still_open_and_this_test_pins_it` pins the divergence so the cross-predicate test stays capable of detecting a disagreement; **when mika#2120 lands, that test must be deleted in the same commit.**
- **Fixtures are frozen, not refreshed.** `crates/mika-agent/tests/fixtures/grooming_bodies/` holds the six measured issue bodies (3 groomed before the fix, 6 after). Refreshing them from GitHub would erase the very forms the predicate must recognize. Read its `README.md` before touching them — it states, line by line, which parts are measured and which are reconstructed.
