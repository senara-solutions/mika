...
Porte de promotion — deux raisons de refus là où il n'y en avait qu'une (mika#2170) :
- **La Phase 2 est la seule entrée de la porte que ne précède pas `is_groomed`, et c'est correct.** Les phases 0 et 1 **posent** `ready` : elles décident d'engager un ticket que personne n'avait engagé, et exiger qu'il soit groomé est la condition même de l'engagement. La Phase 2 **rejoue** un `ready` que quelqu'un d'autre a déjà posé — elle ne décide pas d'engager, elle répare une livraison perdue. Y exiger `is_groomed` désavouerait le `ready` posé à la main par un opérateur et supprimerait le rattrapage de l'**état d'entrée nominal** du pipeline (§ mika#2020 ci-dessus, plus mika#996 qui rend ce secours productif). **Aucune décision de promotion n'a changé** ; `PLAN_PATH_PREFIX` et `is_groomed` sont inchangés, et aucun filtre n'a été ajouté en Phase 2.
- **Ce qui a changé est le diagnostic.** Sur une branche ne portant **aucun** fichier de plan, l'ancien refus annonçait « modifie *n* fichier(s) hors `docs/plans/` » — vrai et sans information, puisque *tout* y est hors grooming — puis prescrivait quoi faire du « travail **partiel** » d'un pilote mort, à un opérateur lisant ça sur sa propre branche. Le nouveau refus nomme la nature de la branche, ne qualifie plus le travail de partiel, et dit que groomer le ticket **sur cette branche** ne lèvera pas le refus (le plan s'ajouterait au code, le refus reviendrait).
- **`reason` est un format de fil, et la scission est datée.** `salvage_work_on_stale_branch` (branche de grooming portant du code) et `stale_branch_without_plan` (branche ne portant aucun plan). Les lignes déjà posées gardent l'ancien nom — les réécrire rendrait faux ce qu'elles disaient. **Un opérateur qui compare de part et d'autre du déploiement doit sommer les deux noms** ; la moitié qui n'a pas changé de nom reste comparable à elle-même. Même raisonnement et même précédent que la scission mika#2361 ci-dessous.
- **`plan_files_count` est posé, pas laissé à dériver.** Nouveau champ de `auto_pull_staleness_measured`, sur **toute** décision, promotions comprises (où il n'y a pas de slug de refus à lire). `null` — jamais `0` — quand la liste n'a pas pu être lue. C'est ce qui rend « cette branche porte-t-elle un plan ? » lisible **sans soustraction de champs** : la condition de réveil d'origine du ticket nommait `non_plan_files`, une liste construite en **filtrant le préfixe hors** d'elle, donc incapable par construction de contenir le frère `docs/plans/` qu'elle cherchait — elle était trivialement vraie de tout refus salvage, y compris sur la branche de grooming nominale. Classe miroir de mika#2272 : là la condition était *insatisfiable*, ici *trivialement satisfiable*, ce qui est pire — elle ne se tait pas, elle répond faux.
- **Surfaces opérateur.**
  ```sql
  -- La distribution des refus de la porte, les deux raisons comptables séparément.
  ...
  ```
  **Régime attendu de `stale_branch_without_plan` : zéro.** La population mesurée le 2026-09-04 (18 tickets ouverts portant un callout `> - **Branch:**`, **zéro** désignant une branche sans plan) était vide et rien ne l'a alimentée depuis. **Une ligne n'est donc pas une panne, c'est un résultat** — la mesure que le ticket attendait depuis cette date. Noter le ticket, la branche et la date.
- **Trois haltes.** (a) *Non-régression, la seule bloquante* : sur 48 h la distribution `outcome = 'promote'` / `'refuse'` est inchangée en volume ; un basculement est la seule chose que ce travail ne doit pas produire — **désarmer par revert, pas par réglage**. (b) *Le nouveau slug porte le trafic nominal* : s'il domine `salvage_work_on_stale_branch`, le discriminant est inversé ou la troncature mord — lire `changed_files_count` **avant** de toucher au prédicat, une branche de grooming portant toujours son plan. (c) *La décision de politique reste réservée* : une population non nulle **n'autorise pas** à élargir `PLAN_PATH_PREFIX` ni à filtrer `is_groomed` en Phase 2 de sa propre autorité — c'est la décision que mika#2140 réserve, elle se prend dans son propre ticket, avec le compte en pièce jointe. **Et le silence ne prouve rien si aucun ticket `ready` ne porte de callout `Branch:`** : vérifier que `outcome = 'promote'` avec `measurement = 'measured'` est non vide avant de conclure quoi que ce soit d'un zéro (classe mika#2205).
