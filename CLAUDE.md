...
- `MIKA_PILOT_STALL_REAP_ENABLED` — whether the reaper may **dispose** of what it detects (default `false`). **Detection is unconditional; only disposition is gated.** Disarmed, the reaper still measures and still writes its `pilot_silent_stall` audit row — it just does not signal the process and does not transition the task. The mode of failure is asymmetric: a false negative costs one dispatch slot, a false positive destroys hours of work in a decision-core worktree, and the threshold's negative control is a sample of size **one**. **Flip condition, dated and concrete:** arm it once `audit_events` carries **at least 3** `pilot_silent_stall` rows whose review confirms **zero** matched a pilot that was still writing (control: the worktree mtimes at measurement time). A single false positive revises the **threshold**; it does not arm the flag. This is the ticket's one remaining manual step.
