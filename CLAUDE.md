...
- **Observability:** "Always instrument, optionally export" pattern. `tracing` spans compiled unconditionally on agent loop (`agent_turn`), team engine (`team_run`, `team_agent_task`), and server handlers. `tower_http::TraceLayer` on Axum router. Feature-gated (`telemetry`) OTLP HTTP export via `mika_common::telemetry::build_otel_layer()` composed into subscriber in server `main()`. `TelemetryGuard` flushes on drop. Team engine emits `TeamEvent::PhaseChanged` and `TeamEvent::AgentStarted` for live dashboard updates. `TeamPhase` enum: Decompose, Execute, Review, Deliver, ReDecompose. Logging functions (`init`, `init_pretty`) accept generic `Option<OL>` OTel layer parameter.
