...
- `cargo test` — Run all tests (~909 tests)
...
- **Long-running exec handlers:** `ToolHandler::Exec` supports `long_running: bool` and `estimated_duration_secs: Option<u64>` fields in `skill.toml`. When `long_running: true` and a `LongRunningContext` is provided (conversation mode only, not silent/team), the executor creates a callback task, injects `__mika_task_id` and `__mika_agent` env vars into the subprocess (MIKA_* env vars scrubbed), spawns the process with `kill_on_drop(false)` and `stdout(Stdio::null())`, and returns immediately with a "task created" message. A background monitor (`spawn_long_running_exec`) awaits the child via `wait()` (no stdout buffering): on success it does nothing (script calls `mika ask --task-id`), on failure it reads capped stderr (`MAX_OUTPUT_LEN` bytes) and marks the task failed. The subprocess PID is recorded; expired tasks get SIGTERM via `kill_orphan_processes()` in the tick loop. Team agents don't use long_running directly — the team engine detects pending grandchild callback tasks and suspends/resumes the run.
