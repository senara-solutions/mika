...
- **LLM:** Multi-provider via `LlmProvider` trait — Anthropic (default, Claude Sonnet 4.6), OpenAI-compatible (OpenAI, Ollama, vLLM, Groq, Together). Provider selected by `claude_model` config with `provider/model` prefix (e.g., `openai/gpt-4o`, `ollama/llama3`). No prefix defaults to Anthropic.
