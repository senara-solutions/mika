...
- **Signal O — RT-005 per-turn token accounting (#1889).** `grep turn_usage $MIKA_SPIRIT_LOG_FILE | jq '{step, stop_reason, mode, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, tool_use_in_turn, status}'` — one INFO line per LLM call in `run_loop` (both success and error arms) plus one for each continuation turn (`step = 4294967295`, the `u32::MAX` sentinel). Ungated by `MIKA_STORE_LLM_CALLS` — the log stream is the RT-005 primary-outcome measurement channel and does not silence when DB persistence is disabled. Emitted on `target: "mika::otel"`. **Prime hard condition #1 (planning-tokens SEUL as primary outcome, covariables descriptive only):** the event carries RAW discriminating dimensions (`step`, `stop_reason`, `mode`, `tool_use_in_turn`) and NEVER a `phase`/`is_planning`/`role` field — the planning-vs-execution boundary is defined by the offline analyzer (brick 5/5), not baked into the thermometer.
