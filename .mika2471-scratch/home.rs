...
mod tests {
    ...
    ///
    /// `label` names the file in every line this returns. It is a parameter
    /// rather than a constant because the guard now walks the whole workspace
    /// (mika#2471): a hard-coded `home.rs` would attribute an offender of
    /// `well_known_agents.rs` to the wrong file, which is worse than no path at
    /// all — it sends the reader to a file that does not carry the defect. The
    /// good-faith control passes `"fabricated.rs"`.
        ...
        while i < lines.len() {
            ...
            match closed_at {
                ...
                None => desyncs.push(format!(
                    "  {}:{} in `{}` — body never closed before end of file",
                    label,
                    signature_at + 1,
                    signature
                )),
    ...
    /// mika#2131).
    ///
    /// **Scope: every `.rs` file under `crates/`** (mika#2471). It was this file
    /// alone until then, matching mika#2073's AC2 verbatim — and six sibling
    /// sites in `mika-agent/src/well_known_agents.rs` carried the same armed
    /// mine, silently, because none of them asserts template content. Widening
    /// the scan and converting those six is one gesture at one commit, in that
    /// order: widened first, the guard is red on sites nobody yet had the
    /// mandate to convert.
    ///
    /// **`tests/` and `src/bin/` are in scope, and that is deliberately stricter
    /// than the race.** An integration test compiles into its own binary, so a
    /// bare `#[test]` there races no setter in `src/`. It is scanned anyway: the
    /// mandate says "workspace", not "the binaries where a setter happens to
    /// live"; passing the tier costs nothing and makes the test honest about the
    /// tier it assumes; and a per-directory exclusion is an allowlist wearing
    /// another name. Today no site falls there.
    ///
    ...
    ///
    /// It reads text, not an AST: an aliased call, a call whose name and `(` sit
    /// on different lines, or a body wrapped in a **helper** that carries no test
    /// attribute all escape it. `well_known_agents.rs`'s `pre_seed_identity` is
    /// exactly that last shape — mika#2471 closes it by hand and names it at the
    /// site, rather than claiming the limit is lifted.
    #[test]
    fn mika2073_no_bare_test_reads_the_tier_from_the_environment() {
        // `rust_sources_under` and **not** `SourceGuard::for_each_under`, which
        // is the same loop plus a "the walk saw nothing" assertion and is the
        // wrong tool here: it hands the callback `production_of(path)`, i.e. the
        // file with its `cfg(test)` regions blanked. A guard built on it would
        // see `tests_seen = 0` in every file and pass green while structurally
        // blind to the only population it exists to read. Do not "simplify" this
        // to that helper; the anchors below are what would catch you, and
        // adjusting them to fit is how this guard dies.
        ...
        for path in &files {
            ...
            let source = std::fs::read_to_string(path)
                .unwrap_or_else(|e| panic!("the guard must be able to read {label}: {e}"));
            let scan = scan_bare_tests_reading_the_tier(&label, &source);
            per_file.insert(label, scan.tests_seen);
            tests_seen += scan.tests_seen;
            offenders.extend(scan.offenders);
            desyncs.extend(scan.desyncs);
        }

        // A scan that silently read nothing is indistinguishable from a clean
        // tree (class mika#2205). Over ~600 files an item count alone no longer
        // detects a drifted root — a walk that reached `mika-agent` only would
        // still see thousands — so the floor is two *named* anchors plus a global
        // one. Each message names the two causes a red can have, because the
        // second is invisible in an otherwise correct path.
        let seen_paths = || {
            per_file
                .keys()
                .take(5)
        ...
        for (anchor, floor) in [
            ("mika-common/src/home.rs", 40usize),
            ("mika-agent/src/well_known_agents.rs", 40usize),
        ] {
            let seen = per_file.get(anchor).copied();
            assert!(
                seen.is_some_and(|n| n >= floor),
                "mika#2073/#2471 — the guard saw {:?} test items in the anchor \
                 `{anchor}` (floor {floor}). Either the walk did not reach it \
                 (root drifted, or the file was renamed or split), or it is \
                 reading a *masked* source — `SourceGuard` blanks test regions, \
                 and a guard fed that reads zero everywhere while looking \
                 healthy. Walked {} file(s) from {}; first paths seen: [{}]. \
                 The fix is to repair the walk or update this anchor to the \
                 file's new name — never to lower the floor.",
                seen,
                files.len(),
                crates_root.display(),
                seen_paths()
        ...
        assert!(
            files.len() >= 400 && tests_seen >= 4000,
            "mika#2073/#2471 — the guard walked {} file(s) and {} test item(s) \
             under {} — too few for this workspace. Either the root points at a \
             sub-tree, or the sources are being read masked (see above). Its \
             green means nothing until both numbers are plausible.",
            files.len(),
            tests_seen,
            crates_root.display()
        );
        ...
        assert!(
            ...
            "mika#2073 — {} non-serial test(s) read `MIKA_AGENT_TIER` off the \
             ...
             above names, and pass the tier the test assumes —\n\
               `bootstrap_with_tier(&home, AgentTier::Default)`\n\
               `bootstrap_agent_with_tier(home, name, AgentTier::Default)`\n\
               `bootstrap_fresh_install_with_tier(home, AgentTier::Default)`\n\
    ...
    ///
    /// **One** of the shapes below does not occur in the sources actually
    /// scanned: the keyed `#[serial(…)]`, whose only four occurrences in the
    /// workspace are the fabricated lines right here. This control is therefore
    /// its **sole** attestation, which is exactly why it cannot be trimmed. The
    /// same holds for the desync self-check's two failure arms, which a
    /// well-formed file never reaches.
    ///
    /// Two shapes left that population when mika#2471 widened the scan from
    /// `home.rs` to all of `crates/`: `#[tokio::test…]` (2 106 items) and
    /// `#[serial_test::serial]` in attribute position (35, including the three
    /// `MIKA_AGENT_TIER` setters) are now traversed on real, rustfmt-shaped
    /// source on every run — and `desyncs = 0` over ~7 800 items is what attests
    /// `attribute_at` and `brace_scan` on them. They are kept here all the same:
    /// this control is what says the walk is *right*, the widened run only says
    /// it does not choke.
    #[test]
