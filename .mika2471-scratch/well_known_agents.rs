...
mod tests {
    ...
    ///
    /// **The tier is posed here, and this site is the scan's blind spot**
    /// (mika#2471). `bootstrap_agent` reads `MIKA_AGENT_TIER` off the *process*
    /// environment, which every thread of this test binary shares with the two
    /// `#[serial]` setters of that variable in `server/mod.rs` and
    /// `server/tier_guard.rs` — and `#[serial]` only sequences its own bearers.
    /// The twelve callers below are bare `#[test]`s, so posing the tier once
    /// here covers all twelve at one gesture.
    ///
    /// It is also the one site of that class that
    /// `mika2073_no_bare_test_reads_the_tier_from_the_environment` cannot see:
    /// the guard walks test *items*, and this is a helper carrying no test
    /// attribute (the "helper-wrapped" limit its own doc-comment names). So the
    /// guard was never red here and its green says nothing about this line —
    /// what attests it is the diff and the green of `-p mika-agent --lib`. A
    /// known blind spot is named, not kept quiet (class mika#2205): if you
    /// revert this to `bootstrap_agent`, **nothing will fail**, and the race
    /// comes back for twelve tests at once.
